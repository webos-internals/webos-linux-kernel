...
#else
	/*
	* PALM TODO: Hack, set the same uart params as in bootie, since the above
	* call to clk_enable return without setting the clk sinc eht RPC is not
	* set yet.
	*/
			    ...
			    struct ktermios *old)
{
	...
	port->ignore_status_mask = termios->c_iflag & (INPCK | IGNPAR);
	port->read_status_mask = termios->c_cflag & CREAD;
...
{
	if (unlikely(ser->type != PORT_UNKNOWN && ser->type != PORT_MSM_SERIAL))
		return -EINVAL;
...
static int __init msm_serial_probe(struct platform_device *pdev)
{
	...
	msm_port->clk = clk_get(NULL, msm_port->clk_name);
	if (unlikely(!msm_port->clk))
		return -ENXIO;
...
#ifdef CONFIG_SERIAL_MSM_NO_UART3_RX_WAKE
	if(port->line != 2)
...
static int __devexit msm_serial_remove(struct platform_device *pdev)
{
	...
	msm_port = UART_TO_MSM(get_port_from_line(pdev->id));
