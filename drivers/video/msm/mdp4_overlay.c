...
void mdp4_overlay_vg_setup(struct mdp4_overlay_pipe *pipe)
{
	...
	if(pipe->solid_fill){
		outpdw(vg_base + 0x1008, 0x00000000);
		outpdw(vg_base + 0x0058, 0);
	}
	else {
...
/*
 * Function to update any of the non-top layers
 *
 * It only updates layer information, but will not push out a frame
 */
int mdp4_overlay_update_layer(
	int layer,
	struct msm_fb_data_type *mfd,
	int memory_id,
	int offset)
...
/*
 * Function to update the top layer, which is fb0
 *
		...
		struct file **pp_src_file, bool push)
{
	...
	*pp_src_file = p_src_file;
	 ...
	 * We do this in mdp4_overlay_update_lcd, but that's not
	 * called in the case where a video layer is active
	...
	if (pipe->mixer_num == MDP4_MIXER1) {
		...
		if (ctrl->panel_mode & MDP4_PANEL_LCDC)
			...
			if (mfd->panel_power_on) {
				...
				/* TODO: As of Qualcomm 2030_08 drop, a new machanism is place
				   to batch up updates from layers to be blended and sent
				   out to the display. Need to investigate*/
