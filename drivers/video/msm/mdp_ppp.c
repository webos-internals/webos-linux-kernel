...
static void rotate_dst_addr_x(struct mdp_blit_int_req *req, struct mdp_regs *regs)
{
...
static void rotate_dst_addr_y(struct mdp_blit_int_req *req, struct mdp_regs *regs)
{
...
static void blit_rotate(struct mdp_blit_int_req *req,
...
static void blit_convert(struct mdp_blit_int_req *req, struct mdp_regs *regs)
{
	...
	} else if( IS_YCRCB(req->bg.format) && IS_RGB(req->src.format)) {
		regs->op |= PPP_OP_CONVERT_YCBCR2RGB | PPP_OP_CONVERT_ON;
	}
}
...
static uint32_t transp_convert(struct mdp_blit_int_req *req)
{
	uint32_t transp = 0;
...
static void blit_blend(struct mdp_blit_int_req *req, struct mdp_regs *regs)
{
	/* TRANSP BLEND */
	...
	} else if(req->bg.format == MDP_ARGB_8888) {
		regs->op |= PPP_OP_ROT_ON | PPP_OP_BLEND_ON |
			PPP_OP_BLEND_DSTPIXEL_ALPHA |
...
static int get_edge_cond(struct mdp_blit_int_req *req, struct mdp_regs *regs)
{
...
static int blit_scale(struct mdp_blit_int_req *req, struct mdp_regs *regs)
{
...
static void get_len(struct mdp_int_img *img, struct mdp_rect *rect, uint32_t bpp,
...
static int valid_src_dst(unsigned long src_start, unsigned long src_len,
			 ...
			 struct mdp_blit_int_req *req, struct mdp_regs *regs)
{
...
int get_img(struct mdp_int_img *img, unsigned long *start,unsigned long *len)
{
	int ret = -1;
	unsigned long vs;


	if(img->smem_start!= 0 && img->smem_len!= 0) {
		*start = img->smem_start;
		*len = img->smem_len;
		return 0;
	}
	else if(!img->filp) {
	...
	if (!get_pmem_addr(img->filp, start, &vs, len))
		ret = 0;
...
static void flush_imgs(struct mdp_blit_int_req *req, struct mdp_regs *regs)
{
#ifdef CONFIG_ANDROID_PMEM
	uint32_t src0_len, src1_len, dst0_len, dst1_len;

	/* flush src images to memory before dma to mdp */
	if(req->src.smem_start != 0 && req->src.smem_len != 0)
	{
		get_len(&req->src, &req->src_rect, regs->src_bpp, &src0_len,
			&src1_len);
		flush_pmem_file(req->src.filp, req->src.offset, src0_len);
	...
	/* flush dst images */
	if(req->dst.smem_start != 0 && req->dst.smem_len != 0)
	{
		get_len(&req->dst, &req->dst_rect, regs->dst_bpp, &dst0_len,
			&dst1_len);
		flush_pmem_file(req->dst.filp, req->dst.offset, dst0_len);
...
static void get_chroma_addr(struct mdp_int_img *img, struct mdp_rect *rect,
...
static int send_blit(struct mdp_blit_int_req *req, struct mdp_regs *regs, unsigned int overlay)
{
	...
	DLOG("SRC Dimensions: height=%d, width=%d\n", req->src_rect.h, req->src_rect.w);
	DLOG("SRC Info: src0=0x%x, src1=0x%x, ystride=0x%x, src_cfg=0x%x, pack=0x%x\n",
		regs->src0, regs->src1, regs->src_ystride, regs->src_cfg, pack_pattern[req->src.format]);
	DLOG("DST Dimensions: height=%d, width=%d\n", req->dst_rect.h, req->dst_rect.w);
	DLOG("DST Info: dst0=0x%x, dst1=0x%x, ystride=0x%x, dst_cfg=0x%x, pack=0x%x\n",
		regs->dst0, regs->dst1, regs->dst_ystride, regs->dst_cfg, pack_pattern[req->dst.format]);
	DLOG("Operation: 0x%x\n", regs->op);

	if(overlay)
	{
		writel(regs->bg0, PPP_ADDR_BG0);
		writel(regs->bg1, PPP_ADDR_BG1);
		writel(regs->bg_ystride, PPP_ADDR_BG_YSTRIDE);
		writel(regs->bg_cfg, PPP_ADDR_BG_CFG);
		writel(pack_pattern[req->bg.format], PPP_ADDR_BG_PACK_PATTERN);

		DLOG("BG Info: bg0=0x%x, bg1=0x%x, ystride=0x%x, bg_cfg=0x%x, pack=0x%x\n",
			regs->bg0, regs->bg1, regs->bg_ystride, regs->bg_cfg, pack_pattern[req->bg.format]);
	}
...
int blit(struct mdp_blit_int_req *req,
	 unsigned long src_start, unsigned long src_len,
	 unsigned long dst_start, unsigned long dst_len,
	 unsigned long bg_start, unsigned long bg_len)
{
	...
	/* setup background image if needed */
	if(bg_len != 0)
	{
		/* set the background image configuration */
		regs.bg_cfg = bg_img_cfg[req->bg.format];

		regs.bg_bpp = bytes_per_pixel[req->bg.format];
		regs.bg0 = bg_start + req->bg.offset;
		regs.bg_ystride = req->bg.width * regs.bg_bpp;
		get_chroma_addr(&req->bg, &req->bg_rect, regs.bg0, regs.bg_bpp,
				regs.bg_cfg, &regs.bg1, &regs.bg_ystride);
		regs.bg0 += (req->bg_rect.x + (req->bg_rect.y * req->bg.width)) *
			    regs.bg_bpp;
		overlay = 1;

		DLOG("bg_start 0x%x, bg_offset 0x%x\n", bg_start, req->bg.offset);
		 
	}

	// TODO: validate background image
...
{
	int ret;
	unsigned long src_start = 0, src_len = 0, bg_start=0, bg_len=0, dst_start = 0, dst_len = 0;
	 ...
	 * safely call put_img */
	if (unlikely(get_img(&req->src, &src_start, &src_len))) {
		printk(KERN_ERR "mdp_ppp: could not retrieve src image from "
				"memory\n");
		return -EINVAL;
	}
	
	get_img(&req->bg, &bg_start, &bg_len);
	
	if (unlikely(get_img(&req->dst, &dst_start, &dst_len))) {
	...
	if (unlikely((req->transp_mask != MDP_TRANSP_NOP ||
		...
		for (i = 0; i < tiles; i++) {
			...
			ret = blit(req, src_start, src_len, dst_start,
				   dst_len, bg_start, bg_len);
			if (ret)
				goto err_bad_blit;
			ret = mdp_ppp_wait();
			if (ret)
	...
	enable_mdp_irq(DL0_ROI_DONE);
	ret = blit(req, src_start, src_len, dst_start, dst_len, bg_start, bg_len);
	if (ret)
		goto err_bad_blit;
	ret = mdp_ppp_wait();
	if (ret)
