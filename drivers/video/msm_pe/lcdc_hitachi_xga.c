...
static unsigned int hitachi_panel_version = WS2;
static struct msm_panel_common_pdata *lcdc_hitachi_pdata;

static int lcdc_hitachi_panel_on(struct platform_device *pdev)
{
	int i = 0;
	/* default to ws2 panel */
	struct hitachi_spi_data *d = init_settings_ws2;

	if (hitachi_panel_version == WS1)
		d = init_settings_ws1;

	for (i = 0; d[i].addr != 0xff || d[i].data != 0xFF; i++)
	{
		hitachi_spi_write(d[i].addr, d[i].data);
		/* we need to wait panel to be stable
					...
					const struct i2c_device_id *id)
{
	...
	pinfo->lcdc.h_front_porch = 216;
	pinfo->lcdc.h_pulse_width = 70;
	pinfo->lcdc.v_back_porch = 13;
	pinfo->lcdc.v_front_porch = 34;
