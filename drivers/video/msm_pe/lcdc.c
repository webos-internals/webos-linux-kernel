...
#define MIPI_DSI_HW_BASE_TEMP	0x04700000
static struct resource msm_mipi_dsi_resources[] = {
	{
		.name   = "mipi_dsi",
		.start  = MIPI_DSI_HW_BASE_TEMP,
		.end    = MIPI_DSI_HW_BASE_TEMP + 0x000F0000 - 1,
		.flags  = IORESOURCE_MEM,
	}
};

static void dsi_pixel_lcdc_clk( int enable)
{
	if (enable) {

		MIPI_OUTP(MIPI_DSI_BASE + 0x0290, 0x7f);
		MIPI_OUTP(MIPI_DSI_BASE + 0x0204,0x80);
		MIPI_OUTP(MIPI_DSI_BASE + 0x0208,0xb1);
		MIPI_OUTP(MIPI_DSI_BASE + 0x020c,0xdd);
		MIPI_OUTP(MIPI_DSI_BASE + 0x0228,0x09);

		MIPI_OUTP(MIPI_DSI_BASE + 0x0200, 0x41);
	} else	{
...
static int lcdc_probe(struct platform_device *pdev)
{
	...
	size =  resource_size(&msm_mipi_dsi_resources[0]);
	mipi_dsi_base =  ioremap(msm_mipi_dsi_resources[0].start, size);
