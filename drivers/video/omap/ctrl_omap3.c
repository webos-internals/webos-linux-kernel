...
static void lcd_controller_enable(struct omap_lcd_controller *ct)
{
	...
	if( ct->lvds_power_enable ) {
		ct->lvds_power_enable(1);
	}
#endif
...
#ifdef CONFIG_MACH_SIRLOIN_3630
	ct->lvds_power_enable     = ctrl_plat->lvds_power_enable;
#ifdef CONFIG_OMAP_DISPLAY_ALREADY_ON_AT_BOOT
	/* it is assumed that lvds power is enabled  */
#else	
	if (ct->lvds_power_enable) {
		ct->lvds_power_enable(1);
	}
