...
static int gpio_input_event(struct input_dev *dev, unsigned int type, unsigned int code, int value)
{
	...
	for(i = 0, ii = ip->info->info; i < ip->info->info_count; i++, ii++) {
		if ((*ii)->event) {
			tmp_ret = (*ii)->event(ip->input_dev, *ii, &ip->state[i], type, code, value);
			if (tmp_ret)
				ret = tmp_ret;
		}
	}
...
static int __init gpio_event_probe(struct platform_device *pdev)
{
	...
	if (event_info == NULL) {
		printk(KERN_ERR "gpio_event_probe: No pdata\n");
		return -ENODEV;
	}
	if (event_info->name == NULL ||
	   event_info->info == NULL ||
	   event_info->info_count == 0) {
		printk(KERN_ERR "gpio_event_probe: Incomplete pdata\n");
		return -ENODEV;
	}
	ip = kzalloc(sizeof(*ip) + sizeof(ip->state[0]) * event_info->info_count, GFP_KERNEL);
	if (ip == NULL) {
		err = -ENOMEM;
		printk(KERN_ERR "gpio_event_probe: Failed to allocate private data\n");
	...
	if (input_dev == NULL) {
		err = -ENOMEM;
		printk(KERN_ERR "gpio_event_probe: Failed to allocate input device\n");
	...
	if (err) {
		printk(KERN_ERR "gpio_event_probe: Unable to register %s input device\n", input_dev->name);
...
{
	printk(KERN_INFO "GPIO Event Driver\n");
