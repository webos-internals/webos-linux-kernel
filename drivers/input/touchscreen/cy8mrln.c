...
cy8mrln_work_handler_old(struct work_struct *work)
{
	...
	else 
	{
		...
		if (dev->timestamp) {
			stamp_size = sizeof(struct timespec);
			ktime_get_ts(&now);
...
cy8mrln_work_handler_new(struct work_struct *work)
{
		...
		CY8MRLN_AUTO_SCAN  == dev->cmd) 
	{
		...
		/* Wake on touch mode */
		if(dev->auto_scan == 1 && dev->scan_rate)
		{
			u16 regval, sensval;
              ...
              unsigned int cmd, unsigned long args)
{
	...
	switch (cmd) {
		...
		case CY8MRLN_IOCTL_START_WOT_SCAN:
		{
			...
			{
				/* no need to enter WOT if pSoC is off */
				printk(KERN_ERR "%s: No need to start WOT; Can't start WOT.\n", DRIVER);
				rc = -EINVAL;
				goto Done;
			}
