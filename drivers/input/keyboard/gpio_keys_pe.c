...
RAW_NOTIFIER_HEAD(diag_key_notifier_list);
EXPORT_SYMBOL(diag_key_notifier_list);
#endif
...
{
	unsigned long flags;
	int i, diag, reboot, console;

	diag = reboot = console = 1;
...
#endif
#ifdef  CONFIG_GPIO_KEYS_DIAG_TRIGGER
		if( button->options & OPT_DIAG_TRIGGER ) {
			if( button->options & OPT_DIAG_TRIGGER_EDGE ) {
				diag &=(btn->state != btn->prev_state);
			} else {
				diag &= btn->state;
			}
		}
...
#endif
#ifdef  CONFIG_GPIO_KEYS_DIAG_TRIGGER
	if( diag || kdev->diag_state != diag) {
		printk("%s: diag event; diag=%d\n", __FUNCTION__, diag);
		kdev->diag_state = diag;
		raw_notifier_call_chain(&diag_key_notifier_list, 
		                         diag, NULL);
	}
#endif
#ifdef  CONFIG_GPIO_KEYS_CONSOLE_TRIGGER
...
#ifdef CONFIG_GPIO_KEYS_TRIGGER
	spin_lock_init(&kdev->keytrigger_lock);
#endif
...
static int gpio_keys_suspend(struct platform_device *pdev, pm_message_t pm_state)
{
	...
	for (i = 0; i < kdev->nbtns; i++) {
		...
		btn->prev_state = (gpio_get_value(button->gpio) ? 1 : 0) ^ button->active_low;
	}
