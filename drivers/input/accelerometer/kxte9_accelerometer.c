...
* 
******************************************************************************/ 
static int kxte9_tilt_cur_pos(u8 tilt_cur, struct kxte9_device_state *state)
{
	...
	if ( (pos <= ORIENTATION_LEFT) && (NULL != state->pdata->orientation_translation_map) ) {
		pos = state->pdata->orientation_translation_map [pos];
	}
...
void kxte9_translate_xyz (struct kxte9_device_state *state, int *x_gravity, int * y_gravity, int * z_gravity)
{
	...
	if (NULL != state->pdata->xyz_translation_map) {
...
kxte9_read_xyz(struct work_struct *work) 
{
	...
	if (int_src&TILT_INT)
	{
		...
		tilt_pos = kxte9_tilt_cur_pos(val, state);
