...
static const char *debug_log_file = "/var/log/reports/misc/led_debug";
...
static int dump_log_file(struct lp8501_device_state *p_state, 
			u8 reg,
			int count)
{
	struct file *file;
	...
	int len;
    	...
    	file = filp_open(debug_log_file, flags, mode);

	if (IS_ERR(file)) {
		printk("lp8501: Failed to open path %s because of %ld\n", 
			debug_log_file, (unsigned long)file);
		printk(KERN_ERR"lp8501: Expected CONFIG: 0x%x; got 0x%x\n", 
			p_state->config_reg, reg);
		goto done;
    	}

	memset(buf, 0 , sizeof(buf));
	len = sprintf(buf, "LED: bad register read after %d times: 0x%2x\n",count,reg);
...
start_engine(struct lp8501_device_state *p_state, int eng)
{
	...
	//Force log dump and restore expected value for CONFIG sliently
	...
	if(dump) {
		dump_log_file(p_state, reg, dbg_read_count);
		dump = false;
	}
