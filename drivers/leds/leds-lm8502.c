...
int lm8502_strobe_input_enable(uint8_t enable)
{
	u8 reg;
	int rc = 0;

	rc = lm8502_i2c_read_reg(lm8502_state->i2c_dev, CONFIG_REG1, &reg);
	if (rc < 0)
		return rc;
	rc = lm8502_i2c_write_reg(lm8502_state->i2c_dev, CONFIG_REG1, 
		enable ? (reg & ~0x04) : (reg | 0x04));

	return rc;
}

int lm8502_set_led_current(uint8_t led_index, uint8_t curr_bits)
{
	int rc = 0;

	if ((led_index > MAX_LED_NUMBER) || (led_index < 1))
		return -EINVAL;

	rc = lm8502_i2c_write_reg(lm8502_state->i2c_dev, D1_CURRENT_CTRL + led_index - 1, curr_bits);
...
ssize_t lm8502_vib_start (u32 start)
{
	if (!lm8502_state) {
		return -EINVAL;
	}
	if (!lm8502_state->vib_enable) {
		printk(KERN_ERR "Vibrator must be enabled before use\n");
		return -EINVAL;
	}

	lm8502_state->vib_start = (u8)start;

	if (start) {
		lm8502_i2c_write_reg(lm8502_state->i2c_dev, HAPTIC_FEEDBACK_CTRL,
			0x02 + !!(lm8502_state->pdata.vib_invert_direction ^ lm8502_state->vib_direction));
	} else {
		lm8502_i2c_write_reg(lm8502_state->i2c_dev, HAPTIC_FEEDBACK_CTRL, 0);
	}
	return 0;
}
...
ssize_t lm8502_vib_direction (u32 dir)
{
	if (!lm8502_state) {
		return -EINVAL;
	}
	...
	if (lm8502_state->vib_direction != (u8)dir) {
		lm8502_state->vib_direction = (u8)dir;
        if(lm8502_state->vib_start)
            lm8502_i2c_write_reg(lm8502_state->i2c_dev, HAPTIC_FEEDBACK_CTRL,
                0x02 + !!(lm8502_state->pdata.vib_invert_direction ^ lm8502_state->vib_direction ));
	}
	return 0;
...
ssize_t lm8502_vib_duty_cycle (u32 duty_cycle)
{
	if (!lm8502_state) {
		return -EINVAL;
	}
	...
	lm8502_i2c_write_reg(lm8502_state->i2c_dev, HAPTIC_PWM_DUTY_CYCLE, (duty_cycle * 255) / 100);
	lm8502_state->vib_duty_cycle = (u8)duty_cycle;
	return 0;
}
				...
				const char *buf, size_t count)
{
	...
	ret = lm8502_vib_duty_cycle (duty_cycle);
	return (ret ? ret : count);
}
...
ssize_t lm8502_vib_enable (u32 enable)
{
	if (!lm8502_state) {
		return -EINVAL;
	}

	lm8502_state->vib_enable = (u8)enable;

	if (enable) {
		/* Logic to enable follows */
		if (lm8502_state->flash_enable) {
			printk(KERN_ERR "Vibrator and flash cannot be enabled together\n");
			return -EINVAL;
		}

		lm8502_state->pdata.select_vibrator(lm8502_state->i2c_dev);
		lm8502_i2c_write_reg(lm8502_state->i2c_dev, HAPTIC_FEEDBACK_CTRL, 0);
	}
	return 0;
}
		     ...
		     unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		case LM8502_READ_PWM:
		{
			...
			u8 addr;
			if(copy_from_user(&pwm, (struct read_pwm *)arg, sizeof(struct lm8502_read_pwm)))
			{
...
static int lm8502_i2c_probe(struct i2c_client *client, const struct i2c_device_id *dev_id)
{
	...
	printk(KERN_INFO "LM8502 probe called\n");
...
//#else
	ret = gpio_request(GPIO_PE5, "LM8502 enable");
	if (ret != 0) {
		printk(KERN_ERR "LM8502: Failed to get GPIO for enable line. Error code = %d\n", ret);
		goto err2;
	}
	ret = gpio_direction_output(GPIO_PE5, 1);
	if (ret != 0) {
		printk(KERN_ERR "LM8502: Failed to set enable line for output. Error code = %d\n", ret);
		goto err3;
	}
	gpio_set_value(GPIO_PE5, 1);
	printk(KERN_ERR "LM8502: Set enable line.\n");
#endif
	...
	/* Configure power savings mode if specified in the board file*/
	lm8502_i2c_read_reg(client, MISC, &reg);
	lm8502_i2c_write_reg(client, MISC, reg | (pdata->power_mode));
	...
	for (i = 0,leds = pdata->leds; i < pdata->nleds; i++)
	{
		...
		if(leds[i].hw_group != HW_GRP_NONE)
		{
			...
			{
				lm8502_i2c_write_reg(client, leds[i].led_list[j].control_addr,
						    (leds[i].hw_group << 6) );
			}
		...
		{
			printk(KERN_INFO "LM8502: Turning %s on to default %d duty cycle\n",
...
#ifdef CONFIG_LM8502_DBG
	if ((ret = lm8502_dbg_register_i2c_client(state->i2c_dev)))
		goto err5;
#endif // CONFIG_LM8502_DBG
	...
	//disable external strobe input
	lm8502_i2c_write_reg(client, CONFIG_REG1, 0x06);
	return 0;
...
#ifdef CONFIG_PM
static u8 torch_reg = 0;
static u8 flash_reg = 0;
static u8 haptics_pwm_reg = 0;
...
static int lm8502_i2c_suspend(struct i2c_client *client, pm_message_t pm_state)
{
	...
	lm8502_i2c_read_reg(state->i2c_dev, TORCH_BRIGHTNESS, &torch_reg);
	lm8502_i2c_read_reg(state->i2c_dev, FLASH_BRIGHTNESS, &flash_reg);
	lm8502_i2c_read_reg(state->i2c_dev, HAPTIC_PWM_DUTY_CYCLE, &haptics_pwm_reg);

	lm8502_i2c_write_reg(state->i2c_dev, TORCH_BRIGHTNESS, 0x0);
	lm8502_i2c_write_reg(state->i2c_dev, FLASH_BRIGHTNESS, 0x0);
	lm8502_i2c_write_reg(state->i2c_dev, HAPTIC_PWM_DUTY_CYCLE, 0x0);
...
static int lm8502_i2c_resume(struct i2c_client *client)
{
	...
	lm8502_i2c_write_reg(state->i2c_dev, TORCH_BRIGHTNESS, torch_reg);
	lm8502_i2c_write_reg(state->i2c_dev, FLASH_BRIGHTNESS, flash_reg);
	lm8502_i2c_write_reg(state->i2c_dev, HAPTIC_PWM_DUTY_CYCLE, haptics_pwm_reg);
...
static int __init lm8502_module_init(void)
{
	...
	printk("LM8502 module init called\n");
...
static int lm8502_debug_reg_val_write(void *data, u64 val)
{
	struct lm8502_device_state *state = (struct lm8502_device_state *)data;

	if(val > 0xFF) {
		printk("lm8502_debug: value too large\n");
		return -1;
	}

	printk("write: reg=0x%x, value=0x%x\n", reg, (u8)val);
	lm8502_i2c_write_reg(state->i2c_dev, reg, (u8)val);

	return 0;
}

static int lm8502_debug_reg_val_read(void *data, u64 *val)
{
	struct lm8502_device_state *state = (struct l8502_device_state *)data;
	u8 out = 0;

	lm8502_i2c_read_reg(state->i2c_dev, reg, &out);
...
static int lm8502_debug_reg_set(void *data, u64 val)
{
	if(val > 0xFF) {
		printk("lm8502_debug: value too large\n");
		return -1;
	...
	printk("lm8502_debug: set reg to 0x%x\n",reg);

	return 0;
}

static int lm8502_debug_reg_get(void *data, u64 *val)
{
	*val = (u64)reg;
...
static int lm8502_debug_reg_dump(void *data, u64 *val)
{
	struct lm8502_device_state *state = (struct l8502_device_state *)data;
	u8 out = 0;
	u8 regs[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0F, 0x10,
		0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1D, 0x21, 0x22, 0x26, 0x27,
		0x28, 0x29, 0x2A, 0x2B, 0x2C, 0x2D, 0x2F, 0x35, 0x36, 0x37, 0x38, 0x3A, 0x3B, 0x3C, 0x3D, 0x41,
		0x42, 0x48, 0x49, 0x4A, 0x4C, 0x4D, 0x4F, 0xA0, 0xB0, 0xC0, 0xD0, 0xE0, 0xF0};
	int i=0;
	int end = sizeof(regs)/sizeof(u8);

	for(i=0; i < end; i++) {
		lm8502_i2c_read_reg(state->i2c_dev, regs[i], &out);
		printk("read: reg=0x%x, value=0x%x\n", regs[i], out);
	}
...
DEFINE_SIMPLE_ATTRIBUTE(lm8502_reg_fops, lm8502_debug_reg_get, lm8502_debug_reg_set, "%llu\n");
DEFINE_SIMPLE_ATTRIBUTE(lm8502_val_fops, lm8502_debug_reg_val_read, lm8502_debug_reg_val_write, "%llu\n");
DEFINE_SIMPLE_ATTRIBUTE(lm8502_dump_fops, lm8502_debug_reg_dump, NULL, "%llu\n");

static int __init lm8502_debug_init(void)
{
	struct dentry *dent;

	dent = debugfs_create_dir("lm8502", 0);
	if (IS_ERR(dent))
		return 0;

	(void) debugfs_create_file("reg", 0644, dent, lm8502_state, &lm8502_reg_fops);
	(void) debugfs_create_file("val", 0644, dent, lm8502_state, &lm8502_val_fops);
	(void) debugfs_create_file("dump", 0644, dent, lm8502_state, &lm8502_dump_fops);

	return 0;
}

device_initcall(lm8502_debug_init);
#endif
