...
#ifdef CONFIG_TIMEREMOTE_SET_ENABLED
...
#define MSMRTC_DEBUG_MSGS 0
#if MSMRTC_DEBUG_MSGS
...
static int msmrtc_update_sleep_time(void);
...
msmrtc_timeremote_set_time(struct device *dev, struct rtc_time *tm)
{
	...
	if (tm->tm_year < 1970) {
		printk(KERN_ERR "%s : year < 1970 \n", __func__);
		return -EINVAL;
	}

	DLOG("Set RTC date/time: %.2u/%.2u/%.4u %.2u:%.2u:%.2u (%.2u)\n",
...
msmrtc_timeremote_set_time(struct device *dev, struct rtc_time *tm)
{
	printk(KERN_WARNING "%s RTC set_time is not implemented!\n", __FUNCTION__);
	return -ENOSYS;
}
#endif // CONFIG_TIMEREMOTE_SET_ENABLED
...
msmrtc_timeremote_read_time(struct device *dev, struct rtc_time *tm)
{
	...
	DLOG ("Current RTC date/time : %.2u/%.2u/%.4u %.2u:%.2u:%.2u (%.2u)\n",
...
msmrtc_virtual_alarm_set(struct device *dev, struct rtc_wkalrm *a)
{
	...
	DLOG ("%.2u/%.2u/%.4u %.2u:%.2u:%.2u (%.2u)\n",  a->time.tm_mon + 1, a->time.tm_mday, a->time.tm_year + 1900, a->time.tm_hour, a->time.tm_min, a->time.tm_sec, a->time.tm_wday);
	...
	msmrtc_update_sleep_time();

	return 0;
}

static int
msmrtc_virtual_alarm_read(struct device *dev, struct rtc_wkalrm *a)
{
	memset(a, 0, sizeof(struct rtc_wkalrm));

	if(rtcalarm_time != 0) {
		rtc_time_to_tm(rtcalarm_time, &a->time);
		a->enabled = 1;
...
static struct rtc_class_ops msm_rtc_ops = {
	...
	.read_alarm	= msmrtc_virtual_alarm_read,
};
...
msmrtc_update_sleep_time(void)
{
	if (rtcalarm_time) {
		...
		DLOG("%s: msm_pm_set_max_sleep_time  set to %d s\n", __func__, diff);
	} else
		msm_pm_set_max_sleep_time(0);
	return 0;
}
...
{
	DLOG ("****** msmrtc_suspend ****** \n");
	return msmrtc_update_sleep_time();
}
