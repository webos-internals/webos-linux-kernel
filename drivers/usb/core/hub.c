	...
	for (i = 0; i < USB_STS_RETRIES && status == -ETIMEDOUT; i++) {
		for(j=0; j<3; j++){
			...
			if (status == 4) {
				...
				if ((port1 == 2) && (
					data->wPortStatus == 0x100 ||
					data->wPortStatus == 0x101))
					dbg_count = 10;
				if (dbg_count > 0) {
					dbg_count--;
					printk(KERN_INFO"%s port=%x, portstatus=%x, portchange=%x\n",
...
static int usb_reset_and_verify_device(struct usb_device *udev)
{
	...
	printk(KERN_INFO"%s udev->state=%x port=%d\n", __func__, udev->state, port1);
	if (udev->state == USB_STATE_NOTATTACHED ||
			udev->state == USB_STATE_SUSPENDED) {
