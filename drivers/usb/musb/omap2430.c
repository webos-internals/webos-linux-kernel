...
int musb_platform_suspend(struct musb *musb)
{
	 ...
	 *     Autoidle clocks       (1 <<  0)
	 */
	OTG_SYSCONFIG_REG = FORCESTDBY | FORCEIDLE | AUTOIDLE;

	/* Enable MSTANDBY to go high. */
...
int musb_platform_resume(struct musb *musb)
{
	 ...
	 *     Autoidle clocks
	 */
	OTG_SYSCONFIG_REG |= SMARTIDLE;

	/* Disable MSTANDBY to go high. */
	OTG_FORCESTDBY_REG &= ~ENABLEFORCE;

	musb->asleep = 0;

	//printk(KERN_DEBUG "@@ %s: finished\n", __func__);
