...
#ifdef CONFIG_USB_GADGET_EVENT
		usb_gadget_event_vbus_draw(100);
#endif
	}
...
static int usb_hw_reset(struct usb_info *ui)
{
	...
	/*
	 * ui->phy_reset() calls msm_ulpi_init() internally.
	 */
	if (ui->phy_reset)
		ui->phy_reset();
...
#else
	/* REVISIT:
	 * pullup+pulldown doesn't work
	 * call usb_reset() for now
	 */
	INFO("reenumerate: calling usb_reset\n");
	usb_reset(ui);
#endif
}
...
#ifdef CONFIG_USB_GADGET_EVENT
	usb_gadget_event_bind((void *)&ui->gadget);
...
int usb_gadget_unregister_driver(struct usb_gadget_driver *driver)
{
	...
	msm_hsusb_set_vbus_state(0);
	INFO("unregister: calling flush_scheduled_work\n");
