...
static DEFINE_RATELIMIT_STATE(driver_ratelimit, 5 * HZ, 5);
#define RL(args...)                                    \
	do {                                               \
		if (__ratelimit(&driver_ratelimit))            \
			args;                                      \
	} while (0)
...
struct usb_info {
	...
	*/
	...
	int wall_charger_max_current_mA;
...
static void dump_regs(struct usb_info *ui)
{
	printk("msm72k_udc: cmd=%08x sts=%08x intr=%08x port=%08x\n",
	       readl(USB_USBCMD),
	       readl(USB_USBSTS),
	       readl(USB_USBINTR),
	       readl(USB_PORTSC));
	printk("msm72k_udc: otgsc=%08x viewport=%08x\n",
	       readl(USB_OTGSC),
...
static int usb_get_max_power(struct usb_info *ui)
{
	...
	if (temp == USB_CHG_TYPE__INVALID && ui->b_max_pow <= 0)
		return -ENODEV;

	if (temp == USB_CHG_TYPE__WALLCHARGER)
		return ui->wall_charger_max_current_mA;
...
static void usb_chg_event(struct usb_info *ui)
{
#ifdef CONFIG_USB_GADGET_EVENT
	if (USB_CHG_TYPE__INVALID != ui->chg_type) {
		enum gadget_event_source_type source;

		switch (ui->chg_type) {
		case USB_CHG_TYPE__SDP:
			source = G_EV_SOURCE_BUS;
			break;
		default:
			source = G_EV_SOURCE_CHARGER;
			break;
		}

		gadget_event_power_state_changed(source, ui->chg_current);
	}
	else {
		gadget_event_power_state_changed(
				G_EV_SOURCE_NONE, 0);
	}
#endif
}
...
int usb_ept_queue_xfer(struct msm_endpoint *ept, struct usb_request *_req)
{
	...
	if (!ui->online && (ept->num != 0)) {
		...
		spin_unlock_irqrestore(&ui->lock, flags);
		RL(INFO("usb_ept_queue_xfer() called while offline\n"));
		return -ESHUTDOWN;
	}
	...
	for (;;) {
		...
		case USB_STATE_IDLE:
			if (flags & USB_FLAG_START) {
				...
				struct msm_otg *otg = to_msm_otg(ui->xceiv);
#if 0
				/* comment out this shortcut to make sure
				   going to low power mode */
				if (!_vbus) {
...
#ifdef CONFIG_USB_GADGET_EVENT
				/* TBD: need to check if we should
				   send a power state event here */
#endif
...
static int msm72k_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.platform_data) {
		...
		ui->wall_charger_max_current_mA = pdata->wall_charger_max_current_mA;
	}
