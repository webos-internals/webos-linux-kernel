...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,24)
static int
pehci_hcd_ctrl_bulk_urb_dequeue(phci_hcd *hcd, struct ehci_qh *qh, struct urb *urb, int async);
#else
static int
pehci_hcd_ctrl_bulk_urb_dequeue(phci_hcd *hcd, struct ehci_qh *qh, struct urb *urb, int status, int async);
#endif
...
static void pehci_hcd_remove(struct isp1763_dev *isp1763_dev);
static int pehci_hcd_probe(struct isp1763_dev *isp1763_dev, isp1763_id * ids);
		...
		while(!list_empty(&qh->qtd_list)){
				...
				if(qtd){
					...
					switch (usb_pipetype(urb->pipe)) {
						case PIPE_CONTROL:
						case PIPE_BULK:
							pr_info("%s dequeue urb:%p\n", __func__, urb);
							#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,24)
							pehci_hcd_ctrl_bulk_urb_dequeue(hcd, qh, urb, 0);
							#else
							pehci_hcd_ctrl_bulk_urb_dequeue(hcd, qh, urb, urb->status, 0);
							#endif
							break;
						case PIPE_INTERRUPT:
...
void dump_control_urb(struct urb *urb)
{
	struct usb_ctrlrequest *cmd;
	if (!urb || !usb_pipecontrol(urb->pipe) || !urb->setup_packet || !urb->dev )
	{
		return;
	}
	cmd = (struct usb_ctrlrequest *)urb->setup_packet;
	dev_err(&urb->dev->dev, "usb_ctrlrequest: %02x %02x %04x %04x %04x, complete:%p\n", 
			cmd->bRequestType, cmd->bRequest, cmd->wValue, cmd->wIndex, cmd->wLength, urb->complete);
}
	...
	if (ptdstatus &	PTD_STATUS_HALTED) {
		if (ptdstatus &	PTD_XACT_ERROR)	{
			...
			if (PTD_RETRY(ptdstatus)) {
				/*halt the endpoint */
				printk("%s transaction error(%d) --0x%08x --halt the endpoint, urb:%p, ep:%02x, type:%d, %s\n", __func__,
					PTD_RETRY(ptdstatus), ptdstatus, urb, urb->ep->desc.bEndpointAddress, usb_pipetype(urb->pipe), usb_pipein(urb->pipe)?"in":"out");
				dump_control_urb(urb);
				urb->status = -EPIPE;
			} else {
				/*protocol error */
				printk("%s transaction error(%d) --0x%08x --protocol error, urb:%p, ep:%02x, type:%d, %s\n", __func__,
					PTD_RETRY(ptdstatus), ptdstatus, urb, urb->ep->desc.bEndpointAddress, usb_pipetype(urb->pipe), usb_pipein(urb->pipe)?"in":"out");
				dump_control_urb(urb);
				urb->status = -EPROTO;
			}
...
#endif
{
	...
	while (donetoclear) {
		...
		if (!list_empty(&qh->qtd_list))
			...
			qtd = NULL;
		if (qtd!=td_ptd_map->qtd)
		{
			pr_err("#### %s: td_ptd_map->qtd(%p) != qh(%p)\n", __func__, td_ptd_map->qtd, qtd);
		}
		if (!qtd)
		{
			pr_err("#### %s: qtd is NULL\n", __func__);
			goto exit;
		}
		if (qtd->state & QTD_STATE_NEW)	{
		...
		if (qtd->state & QTD_STATE_LAST) {
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,24)
			pehci_hcd_urb_delayed_complete(hcd, qh, urb, td_ptd_map, regs);
#else
...
#endif
{
	...
	/*Clear the interrupt*/
	isp1763_reg_write16(dev, HC_INTERRUPT_REG, dev->int_reg & irq_mask);
...
/*-----------------------------------------------------------*/
	...
	if (intr & HC_MSOF_INT ) {
		...
		while (!list_empty(&pehci_hcd->cleanup_urb.urb_list)) {
			struct isp1763_async_cleanup_urb *tmp;

			tmp = list_first_entry(&pehci_hcd->cleanup_urb.urb_list,
				struct isp1763_async_cleanup_urb, urb_list);
			list_del(&tmp->urb_list);
			spin_unlock(&pehci_hcd->lock);
...
#endif
{
	...
	urb_priv = (urb_priv_t *) urb->hcpriv;
        if(!urb_priv){
		printk("%s urb_priv is NULL,use_count:%d,reject:%d\n",__func__,
			atomic_read(&urb->use_count), atomic_read(&urb->reject));
		goto done;
	}
	if (!urb->ep || !urb->ep->hcpriv)
	{
		printk("%s ep->hcpriv=NULL, urb:hcpriv->qh:%p, use_count:%d, reject:%d\n",__func__,
			urb_priv->qh, atomic_read(&urb->use_count), atomic_read(&urb->reject));
		goto done;
	}
	switch (usb_pipetype(urb->pipe)) {
	case PIPE_CONTROL:
...
#else
	#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,24)
		pehci_hcd_ctrl_bulk_urb_dequeue(hcd, qh, urb, 1);
	#else
		pehci_hcd_ctrl_bulk_urb_dequeue(hcd, qh, urb, status, 1);
	#endif
#endif
		break;
		 ...
		 u16 wIndex, char *buf,	u16 wLength)
{
	...
	case ClearPortFeature:
		...
		case USB_PORT_FEAT_C_SUSPEND:
			{
				struct isp1763_dev *dev = hcd->dev;
				dev->resume = 0;
			}
			break;
...
static int pehci_hcd_suspend_late(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct isp1763_dev *isp1763_dev = pdev->dev.platform_data;

	if (isp1763_dev->resume)
	{
		pr_info("%s: remote wake up, break suspend process\n", __func__);
		return -EAGAIN;
	}
	return 0;
}

static struct dev_pm_ops pehci_hcd_driver_pm_ops = {
	.suspend_noirq = pehci_hcd_suspend_late,
};

/*probe platform device */
static int
pehci_hcd_platform_probe(struct platform_device *pdev)
{
	struct isp1763_dev *isp1763_dev = pdev->dev.platform_data;
	int result = 0;

	if ( (result = pehci_hcd_probe(isp1763_dev, pehci_hcd_pci_driver.id))!=0 )
	{
		goto err;
	}
	else
	{
		...
		{
				pr_info("%s: register_chrdev return major number %d\n", __func__, result);
				result = 0;
		}
...
static int
pehci_hcd_platform_remove(struct platform_device *pdev)
{
	struct isp1763_dev *isp1763_dev = pdev->dev.platform_data;
	pehci_hcd_remove(isp1763_dev);
	return 0;
}

static struct platform_driver pehci_hcd_platform_driver = {
	.probe  = pehci_hcd_platform_probe,
	.remove = pehci_hcd_platform_remove,
	.driver = {
		.name  = hcd_name,
		.owner = THIS_MODULE,
		.pm    = &pehci_hcd_driver_pm_ops,
	},
};
...
{
#ifdef NON_PCI
	struct device *dev = isp1763_dev->dev;
#else /* PCI */
	...
	isp1763_dev->driver = &pehci_hcd_pci_driver;
...
pehci_hcd_resume(struct	isp1763_dev *dev)
{
	...
	isp1763_reg_write16(dev, HC_INTENABLE_REG,0); //0xD6
	isp1763_reg_write32(dev,HC_INTERRUPT_REG_EHCI,0x4); //0x94 //0->4
...
pehci_hcd_suspend(struct isp1763_dev *dev)
{
	...
	temp &= ~(PORT_CSC);    //0x2;
	temp &= ~(PORT_RESUME); //0x40;		/*force port resume*/
	temp |= PORT_SUSPEND;   //0x80;		/*suspend*/
...
//	struct pci_dev *dev = isp1763_dev->pcidev;
	...
	if (!usb_hcd) {
		pr_err("%s: Error: usb_hcd is NULL\n", __func__);
		return;
	}

	disable_irq_wake(isp1763_dev->irq);
	...
	isp1763_reg_write32(isp1763_dev, HC_INTERRUPT_REG_EHCI, 0);
	isp1763_reg_write16(isp1763_dev, HC_INTERRUPT_REG, 0);
	isp1763_reg_write16(isp1763_dev, HC_INTENABLE_REG,0);

	temp = isp1763_reg_read16(isp1763_dev, HC_USBCMD_REG, 0);
	temp &= ~0x01;		/* stop the controller first */
	isp1763_reg_write16(isp1763_dev, HC_USBCMD_REG, temp);

	isp1763_reg_write32(isp1763_dev, HC_HW_MODE_REG,0);

	isp1763_reg_write32(isp1763_dev, HC_POWER_DOWN_CONTROL_REG, 0xffff0800);
	...
	pr_info("%s: complete remove_complete: %p\n", __func__, isp1763_dev->remove_complete);
	complete(isp1763_dev->remove_complete);
...
pehci_module_init(void)
{
	...
	/*register driver */
	result = platform_driver_register(&pehci_hcd_platform_driver);
	if (!result) {
