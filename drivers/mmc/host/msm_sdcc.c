...
void msmsdcc_debug_dumpregs(struct mmc_host *mmc)
{
	int i;
	struct msmsdcc_host *host = mmc_priv(mmc);

	pr_info("%s: ====SDCC registers===\n", mmc_hostname(host->mmc));
	for (i = 0; i < 18; i++) {
...
EXPORT_SYMBOL(msmsdcc_debug_dumpregs);

static ssize_t
set_dumpregs(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count)
{
	...
	if (value)
		msmsdcc_debug_dumpregs(mmc);
	spin_unlock_irqrestore(&host->lock, flags);
	return count;
}

static DEVICE_ATTR(dumpregs, S_IRUGO | S_IWUSR,
		NULL, set_dumpregs);
		 ...
		 unsigned int status)
{
	...
	msmsdcc_debug_dumpregs(host->mmc);
...
static void msmsdcc_debug_host(struct mmc_host *mmc)
{
	struct msmsdcc_host *host = mmc_priv(mmc);
	unsigned long flags;
	u32 status;
	...
	if (host->curr.cmd != NULL) {
		pr_err("%s: cmd opcode 0x%x\n",
			 __func__, host->curr.cmd->opcode);
	}

	pr_err("%s: xfer_size %d xfer_remain %d data_xfered %d\n",
		__func__, host->curr.xfer_size,
		host->curr.xfer_remain, host->curr.data_xfered);
...
/* If there is sdio wake irq comes in between msmsdcc_pm_suspend
 * and msmsdcc_pm_suspend_late, the driver will prevent Apps from
 * going into suspend.
 */
int msmsdcc_pm_suspend_late(struct device *dev)
{
	struct mmc_host *mmc = dev_get_drvdata(dev);
	struct msmsdcc_host *host = mmc_priv(mmc);
	...
	if ((mmc->pm_flags & MMC_PM_WAKE_SDIO_IRQ) &&
		...
		pr_info("%s: Detected SDIO wake up IRQ, cannot suspend\n",__func__);
...
static const struct dev_pm_ops msmsdcc_dev_pm_ops = {
	...
	.suspend_noirq   = msmsdcc_pm_suspend_late,
};
