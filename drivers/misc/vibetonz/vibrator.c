...
static int __init vibe_probe(struct platform_device *pdev)
{
	...
	plat_vibrator_register((struct vibrator_platform_data*)(pdev->dev.platform_data));
...
static int __devexit vibe_remove(struct platform_device *pdev)
{
	...
	state = (struct vibe_state*) dev_get_drvdata(&pdev->dev);
