...
#define HSUART_FUNC_LOG_ENABLE		1
	...
	if (NULL != p_context) {
		...
		if ((!p_buffer) || err) {
			...
			if (  free_bytes > available_bytes ) {
				...
				if (p_buffer->write_index < 0) {
					p_buffer->write_index += p_buffer->size;
				}	
...
__tx_get_buffer_cbk(void* p_data )
{
	...
	ret = hsuart_rxtx_get_next_full_buf(&(p_context->tx_lists),
					&p_buffer);	
...
hsuart_flush_rx_queue(struct dev_ctxt* io_p_context)
{
	...
	*/
	...
	for (;;) {
		ret = hsuart_rxtx_get_next_full_buf(
...
struct file_operations hsuart_fops = {
	.llseek  = no_llseek,
	.read    = hsuart_read,
	.write   = hsuart_write,
//	.fsync   = hsuart_fsync,
