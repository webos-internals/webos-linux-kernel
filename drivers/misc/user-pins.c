...
{   
	...
	for (i = 0; i < s->num_pins; i++) {
		...
		grp_sd = sysfs_get_dirent(s->kobj.sd, s->pins[i].attr_grp.name);
		if (grp_sd == NULL) {
			printk(KERN_ERR "user-pins: failed to get sd for %s\n",
					s->pins[i].attr_grp.name);
		} else {
			s->pins[i].pin.sd = sysfs_get_dirent(grp_sd, "irq");
...
static int user_pins_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	for( i = 0; i < dev_ctxt->num_sets; i++ ) {
		...
		for( j = 0; j < pset->num_pins; j++ ) {
			...
			if ((pset->pins[j].pin.options & PIN_IRQ) &&
				(pset->pins[j].pin.irq_handle_mode & IRQ_HANDLE_OFF) &&
				(atomic_read(&pset->pins[j].pin.irqs_during_suspend) != 0)) {

				atomic_set(&pset->pins[j].pin.irqs_during_suspend, 0);
				spin_unlock_irqrestore ( &pins_lock, flags );
	...
	for (i = 0; i < dev_ctxt->num_sets; i++) {
		...
		for (j = 0; j < pset->num_pins; j++) {
			if (pset->pins[j].pin.options & PIN_WAKEUP_SOURCE) {
				...
				if ((pset->pins[i].pin.options & PIN_IRQ) &&
						pset->pins[i].pin.irq_requested &&
						!pset->pins[i].pin.irq_masked) {
