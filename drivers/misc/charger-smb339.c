...
static smb339_current_values smb339_output_current = SMB339_CURRENT_950mA;
...
static int smb339_i2c_probe (struct i2c_client * client, const struct i2c_device_id *dev_id);
...
static int smb339_set_reg(struct i2c_client *client, u8 smb339_current_value)
{
	...
	u8 curr = 0;
	u8 out_curr = 0;
	u8 reg2 = SMB339_FLOAT_VOLTAGE_440;
	...
	if (smb339_current_value == SMB339_CURRENT_DISABLE) {
		...
		if (rc >= 0) {
		    ...
		    smb339_i2c_read_reg(client, SMB339_REG_OUTPUT, &out_curr);
		    printk("smb339: setting output current to %02X\n", smb339_output_current);
		    out_curr = (out_curr & 0x1F) | (smb339_output_current << 5);
		    smb339_i2c_write_reg(client, SMB339_REG_OUTPUT, out_curr);
		} else {
...
static int smb339_set_current(struct i2c_client *client, int current_ma)
{
	...
	//If current is set to 0, summit will be disabled
	if(current_ma == 0) {

		target = &charge_settings[0];
...
static int smb339_i2c_probe (struct i2c_client * client, const struct i2c_device_id *dev_id)
{
	...
	struct smb339_platform_data *pdata = 0;

	SMB339_DEBUG("smb339: probe called\n");

	pdata = (struct smb339_platform_data *)client->dev.platform_data;
	if(pdata) {
		smb339_output_current = pdata->output_current;
		printk(KERN_INFO "smb339: output current: %d\n", smb339_output_current);
	}
