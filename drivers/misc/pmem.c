...
struct pmem_info {
	...
	int garbage_index;
	/* the buddy allocator bitmap for the region indicating which entries
	...
	unsigned cached;
	unsigned buffered;
	/* in all_or_nothing allocator mode the first mapper gets the whole
	 * space and sets this flag */
	unsigned allocated;
...
#define PMEM_BUDDY_ORDER(id, index) (pmem[id].buddy_bitmap[index].order)
...
int update_pmem_index(struct file *file, int index)
{
...
int has_allocation(struct file *file)
{
...
static int pmem_free_buddy_bestfit(int id, int index)
{
	...
	pmem[id].buddy_bitmap[curr].allocated = 0;
...
static int pmem_free_bitmap(int id, int bitnum)
{
	...
	for (i = 0; i < PMEM_NUM_BITMAP_ALLOCATIONS; i++) {
		...
		if (curr_bit == bitnum) {
			const int curr_quanta = pmem[id].bitm_alloc[i].quanta;

			bitmap_bits_clear_all(pmem[id].bitmap,
				curr_bit, curr_bit + curr_quanta);
			pmem[id].bitmap_free += curr_quanta;
...
int pmem_release(struct inode *inode, struct file *file)
{
	struct pmem_data *data = file->private_data;
...
static int pmem_open(struct inode *inode, struct file *file)
{
	...
	DLOG("current %u(%s) file %p(%d) id %d\n",
		current->pid, currtask_name,
		...
		const enum pmem_align align)
{
	...
	pmem[id].buddy_bitmap[best_fit].allocated = 1;
out:
	return best_fit;
}
		...
		const enum pmem_align align)
{
	...
	for (i = start_bit, bitp = pmem[id].bitmap;
...
/* HACK to expose pmem_allocate because video uses it. */
int pmem_allocate(const int id, const unsigned long len)
{
	return pmem_allocator_buddy_bestfit(id, len, PMEM_ALIGN_4K);
}
...
static void pmem_vma_open(struct vm_area_struct *vma)
{
	...
	DLOG("id %d current %u ppid %u file %p count %d\n", id, current->pid,
...
int pmem_mmap(struct file *file, struct vm_area_struct *vma)
{
	struct pmem_data *data = file->private_data;
	int index;
		  ...
		  unsigned long *len, struct file **filp)
{
	...
	if (unlikely(file == NULL)) {
		...
		get_task_comm(currtask_name, current);
		DLOG("filp %p rdev %d current %u(%s) file %p(%d) id %d\n",
...
void pmem_get_size(struct pmem_region *region, struct file *file)
{
...
static long pmem_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	DLOG("current %u(%s) file %p(%d) cmd %#x, id %d\n",
		current->pid, currtask_name,
