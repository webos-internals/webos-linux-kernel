...
struct dev_ctxt {
	...
	int (*suspend_timer)(void *);
	int (*resume_timer)(void *);

	u32 (*read_timer)(void *);
	u32 (*convert_timer)(u32);
};
...
hres_read_tick ( void ) {
	...
	return gdev->read_timer( gdev->timer );
}
 ...
 */
u32   hres_get_delta_usec ( u32 start, u32 end )
{
	return gdev->convert_timer(end - start);
}
...
hres_event_end ( uint ch )
{
	...
	local_irq_save(flags);
	ts = gdev->read_timer( gdev->timer ) - gdev->ch[ch].tstamp;
	gdev->ch[ch].total += ts;
	gdev->ch[ch].count++;
	gdev->ch[ch].active = 0;
	local_irq_restore(flags);
...
hres_evlog_print ( void )
{
	...
	for (i = 0; i < evlog_num; i++, evt++ ) {
		printk ( "%010d (d%10d): %12d (0x%08x)  %12d (0x%08x)  %s\n",
		   gdev->convert_timer(evt->tstamp),
		   gdev->convert_timer(evt->tstamp - last_tstamp),
		   evt->arg1, evt->arg1,
		   evt->arg2, evt->arg2, 
		   evt->type );
                ...
                char *buf )
{
	...
	for ( ch = 0; ch < NUM_CHANNELS; ch++ ) {
		...
		"%02d:  %010u   %4d\n", ch,
		gdev->convert_timer(gdev->ch[ch].total),
		gdev->ch[ch].count
		);
	}
	return len;
...
hres_counter_probe ( struct platform_device  *pdev )
{
	...
	ctxt->pdev = pdev;
	ctxt->init_timer = pdata->init_timer;
	ctxt->release_timer = pdata->release_timer;
	ctxt->read_timer = pdata->read_timer;
	ctxt->convert_timer = pdata->convert_timer;

	ctxt->suspend_timer = pdata->suspend_timer;
	ctxt->resume_timer = pdata->resume_timer;
			
	rc = ctxt->init_timer(&ctxt->timer);
	if( rc )
		goto free_ctxt;
