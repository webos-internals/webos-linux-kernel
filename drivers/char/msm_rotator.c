...
struct msm_rotator_session_list {
...
static int msm_rotator_start(unsigned long arg, struct file *file)
{
	struct msm_rotator_img_info info;
	struct msm_rotator_session_list *l;
	...
	if ((s == MAX_SESSIONS) && (first_free_index != INVALID_SESSION)) {
		...
		*(msm_rotator_dev->img_info[first_free_index]) = info;
		...
		l = kzalloc(sizeof(struct msm_rotator_session_list), GFP_KERNEL);

		if(!l) {
			printk(KERN_ERR "%s : unable to alloc mem for list entry\n", __func__);
			rc = -ENOMEM;
			goto rotator_start_list_error;
		}

		l->session_id = info.session_id;
		l->filp = file;
		list_add(&l->list, &session_list);
...
rotator_start_list_error:
	kfree(msm_rotator_dev->img_info[first_free_index]);
	mutex_unlock(&msm_rotator_dev->rotator_lock);
	return rc;
}
...
static int msm_rotator_finish(unsigned long arg)
{
	...
	int s;
	unsigned int session_id;
	struct msm_rotator_session_list *l, *tmp;
	...
	// delete all list entries that match provided session_id
	list_for_each_entry_safe(l, tmp, &session_list, list) {
...
static int msm_rotator_release(struct inode *inode, struct file *file)
{
	int rc = 0;
	int s = 0;
	unsigned int session_id;
			     ...
			     unsigned cmd, unsigned long arg)
{
	...
	case MSM_ROTATOR_IOCTL_START:
		return msm_rotator_start(arg, file);
