...
struct pn544_info {
	...
	struct regulator_bulk_data regs[REG_COUNT];
	...
	u8 *buf;			/* WRITE related buffers */
	size_t buflen;
	u8 *readbuf;		/* READ related buffers */
	size_t readbuflen;
	size_t bytesinbuf;
	u8 *curbuf;
};

static struct pn544_info *p_pn544_info;

/* XXXRJT
   Currently the upper level linux kernel driver integrates the regulator
   framework. This should most likely be incorporated in the board level
   file instead. The define below disables the calls in the code. It does
   not disable the entries in the data structures
*/
/* #define PN544_REGULATOR_CTRLS

   Original names as provided in the linux kernel driver.
...
/*	XXXRJT
	These reflect names from the schematic.
	XXXRGP
	These should be the names as defined by the vreg driver, not what is in
	the schematic
*/
static const char reg_vdd_io[]	= "VREG_S3A_1V8";
static const char reg_vbat[]	= "VPH_PWR";
static const char reg_vsim[]	= "NFC_UIM_PWR";
...
static int pn544_enable(struct pn544_info *info, int mode)
{
	...
	if (pdata->enable) {
		dev_dbg(&client->dev, "calling enable\n");
		pdata->enable(mode);
	}
...
static void pn544_disable(struct pn544_info *info)
{
	...
	pdata = client->dev.platform_data;
	if (pdata->disable) {
		dev_dbg(&client->dev, "calling disable\n");
		pdata->disable();
	}
...
/*	read a PN544 I2C HCI FRAME
	PN544 FRAME:
	LEN			- 1 byte
	DATA + CRC	- LEN bytes
	the buffer passed in, and it's associate length must be large enough
	to store a complete frame that will be read.
*/
static int pn544_i2c_read(struct i2c_client *client, u8 *buf, int buflen)
{
	...
	dev_dbg(&client->dev, "recv1: %d, len=%d\n", r, len);
	...
	if (1 + len > buflen) { /* len+(data+crc16) */
		dev_dbg(&client->dev, "EMSGSIZE: len=%d, buflen=%d\n",
			len, buflen);
...
/*	read a PN544 I2C Firmware download FRAME
	PN544 FW FRAME:
	LEN		- 2 bytes
	DATA	- LEN bytes
	the buffer passed in, and it's associate length must be large enough
			  ...
			  size_t count, loff_t *offset)
{
	...
	if (irq == PN544_NONE && info->bytesinbuf == 0) {
	...
	if (info->state == PN544_ST_FW_READY) {
		/* the buffer is empty, do a standard read */
		if (0 == info->bytesinbuf) {
			mutex_lock(&info->read_mutex);
			/* Read a complete PN544 HCI FRAME */
			r = pn544_fw_read(info->i2c_dev, info->readbuf,
					info->readbuflen);
			/*	The physical IRQ is guaranteed to go low when a
				frame is read don't need to check for low, just
				set a flag to indicate it's low
			*/
			info->read_irq = PN544_NONE;
			mutex_unlock(&info->read_mutex);
			...
			bufloc = info->readbuf;
			/* there is data to be buffered */
			if (r > count) {
				info->curbuf = info->readbuf + count;
				info->bytesinbuf = r - count;
				r = count;
			}
			/* there is no data to be buffered */
			else {
				info->curbuf = info->readbuf;
				info->bytesinbuf = 0;
			}

		}
		/* the buffer still has some data in it */
		else{
			bufloc = info->curbuf;
			/* there will still be more data in the buffer */
			if (info->bytesinbuf > count) {
				info->curbuf += count;
				info->bytesinbuf -= count;
				r = count;
			}
			/* we're going to empty the buffer */
			else{
				r = info->bytesinbuf;
				info->curbuf = info->readbuf;
				info->bytesinbuf = 0;
		...
		print_hex_dump(KERN_DEBUG, "FW read: ", DUMP_PREFIX_NONE,
			       16, 2, info->readbuf, r, false);

		*offset += r;
		if (copy_to_user(buf, bufloc, r)) {
			r = -EFAULT;
			goto out;
		}
		...
		if (0 == info->bytesinbuf) {
			mutex_lock(&info->read_mutex);
			/* Read a complete PN544 HCI FRAME */
			r = pn544_i2c_read(info->i2c_dev, info->readbuf,
				info->readbuflen);
			/* The physical IRQ is guaranteed to go low when a frame
			 * is read don't need to check for low, just set a flag
			 * to indicate it's low
			*/
			info->read_irq = PN544_NONE;
			mutex_unlock(&info->read_mutex);
...
*/
	...
	struct pn544_info *info = file->private_data = p_pn544_info;
				 ...
				 const struct i2c_device_id *id)
{
	...
	info->readbuflen = max(PN544_MSG_MAX_SIZE, PN544_MAX_I2C_TRANSFER);
	info->readbuf = kzalloc(info->readbuflen, GFP_KERNEL);
	if (!info->readbuf) {
		dev_err(&client->dev,
			"Cannot allocate memory for pn544_info->readbuf.\n");
		r = -ENOMEM;
		goto err_buf_alloc;
	}
	info->bytesinbuf = 0;
	...
	/*	Unsure if this is needed.
		XXXRGP
		I believe this should specify the range of allowed voltages
		for the input in question on the PN544. i.e., for Pvdd, it
		must be 1.8V, but for VBat, the pn544 can handle 2.3V - 5.5V
		depending on the configuration
	*/
	regulator_set_voltage(info->regs[0].consumer, 1800000, 1800000);
	regulator_set_voltage(info->regs[1].consumer, 1800000, 1800000);
#endif
	...
	r = request_threaded_irq(client->irq, NULL, pn544_irq_thread_fn,
				 pdata->irq_trigger_type(), PN544_DRIVER_NAME,
				 info);
	if (r < 0) {
