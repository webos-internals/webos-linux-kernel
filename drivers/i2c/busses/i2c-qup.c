...
enum {
	...
	QUP_I2C_BUS_MASTER         = 1U << 9,
	QUP_I2C_ARB_LOST           = 1U << 4,
};
...
intr_done:
	...
	dev->err = err;
	if (NULL == dev->complete) {
		dev_err(dev->dev, "%s:%s:%d: dev->complete is NULL\n", __FILE__, __FUNCTION__, __LINE__);
	}
	else {
	...
	while (rem) {
		...
		do {
			...
			if (!timeout) {
				...
				if (dev->pdata->msm_i2c_recover &&
					(istatus & QUP_I2C_BUS_MASTER) == 0)
					dev->pdata->msm_i2c_recover();
				writel(1, dev->base + QUP_SW_RESET);
				ret = -ETIMEDOUT;
				goto out_err;
			}
...
qup_i2c_probe(struct platform_device *pdev)
{
	...
	if (pdata->msm_i2c_config_gsbi)
		pdata->msm_i2c_config_gsbi();
