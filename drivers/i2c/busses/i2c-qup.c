...
qup_i2c_interrupt(int irq, void *devid)
{
	...
	dev_dbg(dev->dev, "i2c interrupt handler (status:0x%X errflags:0x%X op_flags:0x%X)\n",
				status, status1, op_flgs);

	if (!dev->msg) {
		dev_err(dev->dev, "dev->msg is null\n");
	...
	if (status & I2C_STATUS_ERROR_MASK) {
		dev_err(dev->dev, "QUP: I2C status flags :0x%x, errflags:0x%X \
			op_flags:0x%X irq:%d\n", status, status1, op_flgs, irq);
		err = -status;
	...
	if (status1 & 0x7F) {
		dev_err(dev->dev, "QUP: QUP status flags :0x%x \
			I2C status flags:0x%x op_flags:0x%x\n", \
			status1, status, op_flgs);
		err = -status1;
		/* Clear Error interrupt if it's a level triggered interrupt*/
		if (dev->num_irqs == 1)
			writel((status1 & QUP_STATUS_ERROR_FLAGS),
...
qup_i2c_poll_writeready(struct qup_i2c_dev *dev)
{
	...
	ktime_get_ts(&expiry);
	timespec_add_ns(&expiry, 100 * NSEC_PER_MSEC);
	...
	while (rem) {
		...
		do {
			...
			timeout = wait_for_completion_timeout(&complete,
					msecs_to_jiffies(1000 /*dev->out_fifo_sz*/));
			if (!timeout) {
