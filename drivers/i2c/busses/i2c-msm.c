...
msm_i2c_pwr_mgmt(struct msm_i2c_dev *dev, unsigned int state)
{
	...
	if (state != 0) {
		...
		if(NULL != dev->pdata->msm_i2c_clock_set_state)
			dev->pdata->msm_i2c_clock_set_state(1);
		clk_enable(dev->clk);
	} else {
		dev_dbg(dev->dev, "I2C_Power: Disable I2C clock(s)\n");
		clk_disable(dev->clk);
		if(NULL != dev->pdata->msm_i2c_clock_set_state)
			dev->pdata->msm_i2c_clock_set_state(0);
	}
}
...
wait_for_int:
 ...
 out_err:
	...
	if(0 != dev->pdata->power_timeout_ms) {
		s_delay = jiffies + msecs_to_jiffies(dev->pdata->power_timeout_ms);
	}	else {
		s_delay = jiffies + 3*HZ;
	}
	mod_timer(&dev->pwr_timer, s_delay);
