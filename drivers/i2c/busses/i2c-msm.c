...
static int msm_i2c_suspend_noirq(struct platform_device *pdev, pm_message_t state)
{
	struct msm_i2c_dev *dev = platform_get_drvdata(pdev);
	 ...
	 */
	if (dev) {
		/* No need to grab mutex to ensure ongoing transaction is over because
		 * we are in suspend_noirq */
		dev->suspended = 1;
		del_timer_sync(&dev->pwr_timer);
		if (dev->clk_state != 0)
			msm_i2c_pwr_mgmt(dev, 0);
...
static int msm_i2c_resume_noirq(struct platform_device *pdev)
{
	struct msm_i2c_dev *dev = platform_get_drvdata(pdev);
	dev->suspended = 0;
	return 0;
...
static struct platform_driver msm_i2c_driver = {
	...
	.suspend_late	= msm_i2c_suspend_noirq,
	.resume_early	= msm_i2c_resume_noirq,
	.driver		= {
		.name	= "msm_i2c",
		.owner	= THIS_MODULE,
	},
