...
int madc_start_conversion(int index)
{
	...
	/* clear LSB and MSB */ 
	ret = twl4030_madc_write(REG_SW1SELECT_LSB, 0x00);
	ret = twl4030_madc_write(REG_SW1SELECT_MSB, 0x00);

	/* Select ADCINx SW1SELECT register */
	if (index < 8)
	{	
		if (ret < 0)
			goto err;

		ret = twl4030_madc_set_bits(TWL4030_MODULE_MADC, (1<<index), REG_SW1SELECT_LSB);
		if (ret < 0)
			goto err;
	} else {
		if (ret < 0)
		...
		ret = twl4030_madc_set_bits(TWL4030_MODULE_MADC, (1<<(index - 8)), REG_SW1SELECT_MSB);
		if (ret < 0)
			goto err;
	}
	...
	wait_for_completion(&convert_done);
