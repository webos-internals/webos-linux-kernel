...
#define MODE_SELECT_STATUS				0x3368
#define STATE_CHANGE_STEP				2
/* Integration Time */
...
#define REG_0x3388					0x3388
#define REG_0x3640					0x3640
#define REG_0x3042					0x3042
...
enum sensor_status {
	e_Sensor_Startup = 0,
	e_Software_Standby = 1,
	e_Unknown_02 = 2,
	e_Unknown_03 = 3,
	e_Streaming_Startup = 4,
	e_Streaming = 5,
	e_Streaming_Shutdown = 6,
};
...
static uint16_t vx6953_stm5m0edof_delay_msecs_stdby;
static uint16_t vx6953_stm5m0edof_delay_msecs_stream = 20;
static uint8_t vx6953_csi_init_count;
...
static int32_t vx6953_write_exp_gain(uint16_t gain, uint32_t line)
{
  ...
  if (vx6953_ctrl->sensormode == SENSOR_PREVIEW_MODE) {
		vx6953_ctrl->my_reg_gain = gain;
		vx6953_ctrl->my_reg_line_count = (uint16_t)line;
	}
...
int vx6953_wait_sensor_state(int state , int step , int timeout)
{
	unsigned short s_stat = 0;
	int dither = 0;

	int rc = vx6953_i2c_read(MODE_SELECT_STATUS, &s_stat, 1);
	if (rc < 0) {
		pr_err("%s i2c read error: %d", __func__, rc);
		return rc;
	}
	...
	while (state != s_stat) {
		timeout -= step;
		if (timeout < 1) {
			pr_err("timeout waiting for state:%d\n", s_stat);
			return -EBUSY;
		}

		/* dithering is intended to alternate between sleep and
		 * delay calls. */
		if (0 == (dither % 3))
			msleep(step);
		else
		...
		rc = vx6953_i2c_read(MODE_SELECT_STATUS, &s_stat, 1);
		if (rc < 0) {
			pr_err("%s i2c read error: %d", __func__, rc);
			return rc;
		}
...
int vx6953_wait_sensor_frame_start(int step , int timeout)
{
	unsigned short frame_cnt = 0;
	int dither = 0;

	int rc = vx6953_i2c_read(0x0005, &frame_cnt, 1);
	if (rc < 0) {
		pr_err("%s i2c read error: %d", __func__, rc);
		return rc;
	}
	...
	while (frame_cnt == 0xFF) {
		timeout -= step;
		if (timeout < 1) {
			pr_err("timeout waiting for frames\n");
			return -EBUSY;
		}

		/* dithering is intended to alternate between sleep
		 * and delay calls to avoid scheduler starvation. */
		if (0 == (dither % 3))
			msleep(step);
		else
			mdelay(step);
		...
		rc = vx6953_i2c_read(0x0005, &frame_cnt, 1);
		if (rc < 0) {
			pr_err("%s i2c write error: %d", __func__, rc);
			return rc;
		}
	}
	return 0;
...
static int32_t vx6953_set_pict_exp_gain(uint16_t gain, uint32_t line)
{
	...
	rc = vx6953_i2c_write_b_sensor(REG_MODE_SELECT,
			MODE_SELECT_STREAM);
	if (rc < 0) {
	...
	rc = vx6953_wait_sensor_state(e_Streaming, STATE_CHANGE_STEP,
		vx6953_stm5m0edof_delay_msecs_stdby);
	if (rc < 0) {
		pr_err("%s : wait state error:%d\n", __func__, rc);
		return rc;
	}

	/* Wait for frames to appear */
	rc = vx6953_wait_sensor_frame_start(STATE_CHANGE_STEP,
		vx6953_stm5m0edof_delay_msecs_stdby);
	if (rc < 0)
		pr_err("%s : frame start error:%d\n", __func__, rc);
	return rc;
...
static int32_t vx6953_sensor_setting(int update_type, int rt)
{
	...
	if (vx6953_ctrl->sensor_type != VX6953_STM5M0EDOF_CUT_2) {
		...
		if (rt == RES_PREVIEW || rt == RES_CAPTURE) {
			...
			rc = vx6953_i2c_write_b_sensor(0x103, 0x01);
			if (rc < 0) {
				pr_err("S/W reset failed\n");
				return rc;
			} else
				CDBG("S/W reset successful\n");

			msleep(10);
			vx6953_csi_init_count = 0;
			...
			rc = vx6953_wait_sensor_state(e_Software_Standby,
				STATE_CHANGE_STEP,
				vx6953_stm5m0edof_delay_msecs_stdby);
			if (rc < 0) {
				pr_err("%s wait sensor error: %d",
						__func__, rc);
				return rc;
			}
		...
		if (rt == RES_PREVIEW || rt == RES_CAPTURE) {
			...
			/* Wait for streaming state */
			rc = vx6953_wait_sensor_state(e_Streaming,
				STATE_CHANGE_STEP,
				vx6953_stm5m0edof_delay_msecs_stdby);
			if (rc < 0) {
				pr_err("%s wait state error: %d",
			...
			/* Wait for frames to appear */
			rc = vx6953_wait_sensor_frame_start(
				STATE_CHANGE_STEP,
				vx6953_stm5m0edof_delay_msecs_stdby);
			if (rc < 0) {
				pr_err("%s wait frame error: %d",
						__func__, rc);
				return rc;
			}
		}
...
static int32_t vx6953_video_config(int mode)
{
	...
	if (vx6953_ctrl->curr_res != vx6953_ctrl->prev_res) {
		...
		rc = vx6953_sensor_setting(UPDATE_PERIODIC, rt);
		if (rc < 0)
			return rc;
	}

  if (mode == SENSOR_PREVIEW_MODE) {
		rc = vx6953_write_exp_gain(vx6953_ctrl->my_reg_gain,
				vx6953_ctrl->my_reg_line_count);
		msleep(10);
		if (rc < 0)
			return rc;
	}
	...
	if (vx6953_ctrl->set_test) {
		rc = vx6953_test(vx6953_ctrl->set_test);
		if (rc < 0)
			return	rc;
	}
