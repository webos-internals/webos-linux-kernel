...
msmv4l2_startstreaming(struct msmv4l2_device *vout)
{
	...
	//Enable msm_fb to switch the video layer on
...
void 
msmv4l2_callback(struct msmv4l2_device *vout)
{
	spin_lock(&vout->qbufcount_lock);
	vout->qbuf_count--;
	spin_unlock(&vout->qbufcount_lock);
...
msmv4l2_fb_update(struct msmv4l2_device *vout, struct v4l2_buffer *buffer)
{
	...
	/* 
	 * In msm7x30, the fb handling of the video pipeline will
	 * no longer use blit to do the rotation. Nevertheless, 
	 */
...
msmv4l2_vidioc_qbuf(struct file *file, struct msmv4l2_fh* fh, void *arg)
{
	...
	if(ret < 0) {
		DLOG("msmv4l2: msmv4l2_fb_update failed.\n");
		vout->qbuf_count--;
