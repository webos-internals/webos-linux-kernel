...
static uint8_t debug_regaddr = 0;
...
static int32_t ov7692_i2c_write_b(uint8_t waddr, uint8_t bdata)
{
	int32_t rc = -EFAULT;
	unsigned char buf[2];
	memset(buf, 0, sizeof(buf));
...
static ssize_t
ov7692_attr_show_regdata(struct device *dev, struct device_attribute *attr,
	char *buf)
{
	uint8_t val;
	int rc;
	...
	if ((rc = ov7692_i2c_read(debug_regaddr, &val,1)) < 0)
		goto exit;
...
static ssize_t
ov7692_attr_store_regdata(struct device *dev, struct device_attribute *attr,
	const char *buf, size_t count)
{
	...
	if ((rc = ov7692_i2c_write_b(debug_regaddr, (uint8_t)val)) < 0)
		goto exit;
...
static ssize_t
ov7692_attr_show_regaddr(struct device *dev, struct device_attribute *attr,
	char *buf)
{
	int rc;
	rc = sprintf(buf, "0x%02x\n", debug_regaddr);
...
static ssize_t
ov7692_attr_store_regaddr(struct device *dev, struct device_attribute *attr,
	const char *buf, size_t count)
{
	int rc;
	unsigned int val;
	if (!sscanf(buf, "0x%02x", &val)) {
		rc = -EINVAL;
		goto exit;
	}
...
static struct device_attribute ov7692_attr_regdata =
	__ATTR(regdata, S_IRUGO|S_IWUGO, ov7692_attr_show_regdata,
		ov7692_attr_store_regdata);

static struct device_attribute ov7692_attr_regaddr =
	__ATTR(regaddr, S_IRUGO|S_IWUGO, ov7692_attr_show_regaddr,
		ov7692_attr_store_regaddr);
...
//	}
	...
	printk(KERN_INFO "ov7692 model_id = 0x%x, 0x%x, 0x%x\n",
	...
	const struct i2c_device_id *id)
{
	...
	if ((rc = device_create_file(&client->dev, &ov7692_attr_regdata)))
		goto probe_failure;

	if ((rc = device_create_file(&client->dev, &ov7692_attr_regaddr)))
		goto probe_failure;
...
static int __exit ov7692_remove(struct i2c_client *client)
{
	...
	device_remove_file(&client->dev, &ov7692_attr_regdata);
	device_remove_file(&client->dev, &ov7692_attr_regaddr);
