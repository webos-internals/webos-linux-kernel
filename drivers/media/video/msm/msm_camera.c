					...
					spinlock_t *pmem_spinlock)
{
	...
	if (!rc) {
		pr_err("%s: failed to find active type %d! table:\n", 
				__func__, pmem_type);
		hlist_for_each_entry_safe(region, node, n, ptype, list) {
			pr_err("\tinfo.type %d active %d\n", region->info.type, 
					region->info.active);
		}
	}
		...
		int clear_active)
{
	...
	pr_err("frame ptov lookup failure! table:\n");
	hlist_for_each_entry_safe(region, node, n, &sync->pmem_frames, list) {
		pr_err("\tframe pyaddr %p vaddr %p y_off %d cbcroff %d fd %d active %d\n",
				(void *)region->paddr, 
				region->info.vaddr, 
				region->info.y_off, 
				region->info.cbcr_off, 
		...
		uint32_t yoff, uint32_t cbcroff, int fd)
{
	...
	pr_err("frame vtop lookup failure! table:\n");
	hlist_for_each_entry_safe(region, node, n, &sync->pmem_frames, list) {
		pr_err("\tframe pyaddr %p vaddr %p y_off %d cbcroff %d fd %d active %d\n",
				(void *)region->paddr, 
				region->info.vaddr, 
		...
		struct msm_vfe_cfg_cmd *cfgcmd)
{
	...
	CDBG("msm_frame_axi_cfg %d\n", cfgcmd->cmd_type);
	switch (cfgcmd->cmd_type) {

	case CMD_AXI_CFG_PREVIEW:
		pmem_type = MSM_PMEM_VIDEO;  /* HAL allocates buffers with video type */
		axi_data.bufnum1 =
		...
		struct msm_frame *pb)
{
	...
	if (pphy != 0) {
		...
		pr_err("%s: msm_pmem_frame_vtop_lookup failed: "
				"buf 0x%p y_off %d cbcr_off %d fd %d\n",
				__func__, 
				(void *)pb->buffer, pb->y_off, pb->cbcr_off, pb->fd);
		rc = -EINVAL;
	}
						...
						void *ts, gfp_t gfp)
{
	...
	case VFE_MSG_OUTPUT_V:
		CDBG("%s: msm_enqueue video frame_q from VPE"
				"y_phy %p cbcr_phy %p\n", __func__,
				vdata->phy.y_phy, vdata->phy.cbcr_phy);
		if (sync->liveshot_enabled) {
