...
static int vfe31_config_axi(int mode, struct axidata *ad, uint32_t *ao)
{
	...
	case OUTPUT_2: {
		...
		regp2 = &(ad->region[0]); /* this is video buffer. */
		...
		for (i = 0; i < 2; i++) {
			...
			*p2 = (regp2->paddr + regp2->info.cbcr_off);
		    ...
		    CDBG("%s: case OUTPUT2 Y = 0x%x, cbcr = 0x%x\n",
			__func__, (regp2->paddr + regp2->info.y_off),
			(regp2->paddr + regp2->info.cbcr_off));
		}
