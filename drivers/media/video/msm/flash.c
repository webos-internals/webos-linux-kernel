...
int msm_camera_flash_user(
	struct msm_camera_sensor_flash_user *user,
	unsigned led_state)
{
	int rc = 0;
	...
	switch (led_state) {
	case MSM_CAMERA_LED_OFF:
		rc = user->set_current(true, 0);
		if (rc < 0) goto exit;
		rc = user->enable_flash(false);
		break;

	case MSM_CAMERA_LED_LOW:
		rc = user->set_current(true, user->low_current);
		if (rc < 0) goto exit;
		rc = user->enable_flash(true);
		break;

	case MSM_CAMERA_LED_HIGH:
		rc = user->set_current(true, user->high_current);
		if (rc < 0) goto exit;
		rc = user->enable_flash(true);
		break;
	...
	struct msm_camera_sensor_flash_data *fdata, unsigned led_state)
{
	...
	case MSM_CAMERA_FLASH_SRC_USER:
		rc = msm_camera_flash_user(&fdata->flash_src->_fsrc.user_src,
			led_state);
		break;
