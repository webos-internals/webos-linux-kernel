...
static int omap34xx_isp_resume(struct omap34xx_isp_data *data)
{
	...
	if ((rc = clk_enable(data->fclk)))
		goto fclk_enable_failed;
...
omap34xx_isp_device_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(data->fclk = clk_get(NULL, "cam_mclk"))) {
		rc = PTR_ERR(data->fclk);
		goto fclk_get_failed;
	}
