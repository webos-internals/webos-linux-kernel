...
#define ISP_REVISION		ISP_REG(0x00)
...
static int omap34xx_isp_resume(struct omap34xx_isp_data *data)
{
	...
	rc = clk_enable(data->csi2_fck);
	if (rc)
		goto csi2_fck_enable_failed;
...
iclk_enable_failed:
	clk_disable(data->csi2_fck);
csi2_fck_enable_failed:
...
omap34xx_isp_device_probe(struct platform_device *pdev)
{
	...
	data->csi2_fck = clk_get(NULL, "csi2_fck");
	if (IS_ERR(data->csi2_fck)) {
		rc = PTR_ERR(data->csi2_fck);
		goto csi2_fck_get_failed;
	}
	...
	/* Enable Interface clocks tmeporarly to access ISP_REVISION reg */
	clk_enable(data->iclk);
	input->isp_revision = omap_readl(ISP_REVISION);
	clk_disable(data->iclk);

	printk(KERN_INFO OMAP34XX_ISP_DRIVER ": found revision=%x\n", input->isp_revision);
	input->mst.parent = pdev;

	if (input->isp_revision >= ISP_REVISION_ISP2P) {
		/*
