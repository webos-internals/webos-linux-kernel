...
#define	OCP_ERR_IRQ_MASK	(0x1 << 29)
#define	MMU_ERR_IRQ_MASK	(0x1 << 28)
#define	OVF_IRQ_MASK		(0x1 << 25)
...
#define SBL_BASE		0x480BD200
#define SBL_REG(off)		(SBL_BASE + (off))
#define SBL_PCR			SBL_REG(0x04)
...
/*
 * Calculation of ratio for 2x binning on 5MP:
 *
 * We know we want to get VGA (640x480), and we know the resizer max
 * possible theoretical input (Preview output).
 *
 * So, what we want to do here is, come up with a ratio value, which is
 * compilant with the HW constraints, and that brings us VGA.
 *
 * NOTE: This is assuming pixel cropping in between preview and resizer is
 * acceptable.
 * 
 * CCDC input:
 *   W: 1296 (was: 2608 / 2 = 1304)
 *   H: 1960 / 2 = 980
 *
 * CCDC output (VP):
 *   W: 1296
 ...
 *   H: 480
 *
 * Now, to avoid loosing aspect ratio, we need to find a common resizing
 * factor for out/in relationship. Also, we need that the resizing factor
 * gives us integer numbers for the input crop area recalculation.
 *
 * That number is: 503, which after being entered in this formula:
 *
 * real_ratio = (256 / (ratio_val + 1))
 *
 * Gives us 32/63, or aprox: 50.79%, and gives us the following input
 * dimensions:
 *
 * W rsz input: 640 / (32/63) = 1260
 * H rsz input: (480 / (32/63)) + 2 = 947
 *
 * And lastly, to have a centered image, we calculate the necessary offsets
 * like this (assuming preview features are all enabled, so above worst case
 * size is actual resizer input size):
 * 
 * W rsz input offset: (1278 - 1260) / 2 = 9
 * H rsz input offset: (970 - 947) / 2 = 11.5 => 11
 */
...
static struct omap34xx_isp_video_format omap34xx_isp_video_formats[] = {
	[0] = { /* 5MP */
		.pix = {
			.width = ISP_BASE_5MP_OUT_WIDTH,
			.height = ISP_BASE_5MP_OUT_HEIGHT,
			.pixelformat =  V4L2_PIX_FMT_UYVY,
			.bytesperline = ALIGN(ISP_BASE_5MP_OUT_WIDTH * 2, 0x20),
			.sizeimage = PAGE_ALIGN(ALIGN(ISP_BASE_5MP_OUT_WIDTH * 2, 0x20) *
				     ALIGN(ISP_BASE_5MP_OUT_HEIGHT, 16)),
			.colorspace = V4L2_COLORSPACE_SRGB,
		},
		.slv = {
			.width = ISP_BASE_5MP_IN_WIDTH,
			.height = ISP_BASE_5MP_IN_HEIGHT,
		...
		.cfg = {
			...
			.ccdc_fmt_horz_fmtlnh = ISP_BASE_5MP_IN_WIDTH,
			.ccdc_fmt_vert_fmtlnv = ISP_BASE_5MP_IN_HEIGHT,
			.ccdc_vp_out_vert_num = ISP_BASE_5MP_IN_HEIGHT - 1,
			.ccdc_vp_out_horz_num = ISP_BASE_5MP_IN_WIDTH,
			.h3a_pcr_rgbpos = RGBPOS_GRBG,
			.prv_pcr_sdrport = 1,
			.prv_pcr_rszport = 0,
			.prv_pcr_ycpos = PCR_YCPOS_YCRYCB,
			/*
			 * Criteria for preview input horizontal size:
			 *
			 * 1. Preview crops 10 pixels for Capture usecase.
			 *
			 * 2. Preview width must be, at least, 4 pixels less
			 * (2 left and 2 right) than CCDC videoport output width.
			 *
			 * That leaves us with the following formulas:
			 * 1. prv_horz_info_eph = ((2592 + 12) - 2) - 1 = 2601
			 * 2. 2604 + 4 <= 2608 ? Yes.
			 * 3. prv_horz_info_sph = (2608 - (2602 + 4)) / 2 = 1
			 */
			.prv_horz_info_sph = 2,
			.prv_horz_info_eph = (ISP_BASE_5MP_OUT_WIDTH + 10) - 1,
			.prv_vert_info_elv = (ISP_BASE_5MP_OUT_HEIGHT + 8) - 1,
			.prv_wadd_offset = ALIGN(ISP_BASE_5MP_OUT_WIDTH * 2,
						 0x20),
	...
	[1] = { /* VGA */
		.pix = {
			...
			.height = 480,
			.pixelformat =  V4L2_PIX_FMT_UYVY,
			.bytesperline = ALIGN(640 * 2, 0x20),
			.sizeimage = ALIGN(640 * 2, 0x20) * 480,
			.colorspace = V4L2_COLORSPACE_JPEG,
		},

		.slv = {
			.width = ISP_BASE_5MPHALF_IN_WIDTH,
			.height = ISP_BASE_5MPHALF_IN_HEIGHT,
			.pixelformat = V4L2_PIX_FMT_SGRBG10,
		},
		...
		.cfg = {
			...
			.ccdc_fmtcfg_vpen = 1,
			.ccdc_fmt_horz_fmtsph = 0,
			.ccdc_fmt_horz_fmtlnh = ISP_BASE_5MPHALF_IN_WIDTH,
			.ccdc_fmt_vert_fmtlnv = ISP_BASE_5MPHALF_IN_HEIGHT,
			.ccdc_vp_out_vert_num = ISP_BASE_5MPHALF_IN_HEIGHT - 1,
			.ccdc_vp_out_horz_num = ISP_BASE_5MPHALF_IN_WIDTH,
			.h3a_pcr_rgbpos = RGBPOS_GRBG,
			.prv_pcr_sdrport = 0,
			.prv_pcr_rszport = 1,
			.prv_pcr_ycpos = PCR_YCPOS_YCRYCB,
			.prv_horz_info_sph = 2,
			.prv_horz_info_eph = (ISP_BASE_5MPHALF_IN_WIDTH - 2) - 1,
			.prv_vert_info_elv = (ISP_BASE_5MPHALF_IN_HEIGHT - 1) - 1,
			.prv_wbsel = WBSEL_GRBG,
			...
			.rsz_in_start_vert_st = 11,
			.rsz_in_start_horz_st = 9,
			.rsz_in_size_vert = 947,
			.rsz_in_size_horz = 1260,
	...
	[2] = { /* 5MP RAW10 */
		...
		.cfg = {
			...
			.ccdc_fmtcfg_vpen = 1,
			.ccdc_fmt_horz_fmtsph = 0,
			.ccdc_fmt_horz_fmtlnh = ISP_BASE_5MP_OUT_WIDTH,
			.ccdc_fmt_vert_fmtlnv = ISP_BASE_5MP_OUT_HEIGHT,
			.ccdc_vp_out_vert_num = ISP_BASE_5MP_OUT_HEIGHT - 1,
			.ccdc_vp_out_horz_num = ISP_BASE_5MP_OUT_WIDTH,
			/*
			 * The ccdc_horz_info_nph and ccdc_vert_lines_nlv are
			 * substracted by 1 because they're starting from 1, see
			 * TRM for details.
			 */
			.ccdc_horz_info_nph = ISP_BASE_5MP_OUT_WIDTH - 1,
			.ccdc_vert_lines_nlv = ISP_BASE_5MP_OUT_HEIGHT - 1,
			.ccdc_hsize_off_lnofst = ALIGN(ISP_BASE_5MP_OUT_WIDTH * 2,
						 0x20),
			.h3a_pcr_rgbpos = RGBPOS_GRBG,
		},
	},
	[3] = { /* 1296x980 RAW10 */
		.pix = {
			...
			.height = ISP_BASE_5MPHALF_IN_HEIGHT,
			.pixelformat =  V4L2_PIX_FMT_SGRBG10,
			.bytesperline = ALIGN(ISP_BASE_5MPHALF_IN_WIDTH * 2,
					      0x20),
			.sizeimage = ALIGN(ISP_BASE_5MPHALF_IN_WIDTH * 2,
					   0x20) * ISP_BASE_5MPHALF_IN_HEIGHT,
			.colorspace = V4L2_COLORSPACE_SRGB,
		},
		...
		.cfg = {
			 ...
			 * TRM for details.
			 */
			.ccdc_horz_info_nph = ISP_BASE_5MPHALF_IN_WIDTH - 1,
			.ccdc_vert_lines_nlv = ISP_BASE_5MPHALF_IN_HEIGHT - 1,
			.ccdc_hsize_off_lnofst = ALIGN(ISP_BASE_5MPHALF_IN_WIDTH * 2,
						 0x20),
			.h3a_pcr_rgbpos = RGBPOS_GRBG,
		},
	},
...
omap34xx_isp_video_isr(struct omap34xx_isp_v4l2_device *dev)
{
	...
	irq0 &= HS_VS_IRQ_MASK | RSZ_DONE_IRQ_MASK | PRV_DONE_IRQ_MASK
		| CCDC_VD0_IRQ_MASK | OCP_ERR_IRQ_MASK | MMU_ERR_IRQ_MASK
		| OVF_IRQ_MASK;
	...
	SPEW(3, "ISP_IRQSTATUS=0x%08X(%s,%s,%s,%s)\n", irq0,
		...
		OCP_ERR_IRQ_MASK & irq0 ? __stringify(OCP_ERR) : "",
		MMU_ERR_IRQ_MASK & irq0 ? __stringify(MMU_ERR) : "",
		OVF_IRQ_MASK & irq0 ? __stringify(OVF) : "");
	...
	if (OCP_ERR_IRQ_MASK & irq0)
		printk(KERN_ERR "***OCP_ERR_IRQ_MASK\n");

	if (MMU_ERR_IRQ_MASK & irq0)
		printk(KERN_ERR "***MMU_ERR_IRQ_MASK\n");

	if (OVF_IRQ_MASK & irq0) {
		u32 sbl_pcr = omap_readl(SBL_PCR);

		sbl_pcr &= 0x7FF0000;
		omap_writel(sbl_pcr, SBL_PCR);
		printk(KERN_ERR "***OVF_IRQ_MASK\n");
		printk(KERN_ERR "***SBL_PCR: %x\n", sbl_pcr);
	}
...
__omap34xx_isp_video_config(struct omap34xx_isp_video_data *data)
{
	...
	/* Prepare ISP Preview input size re-configuration, no additional cropping vals */
	if (data->vfmt->cfg.prv_pcr_sdrport) {
		data->vfmt->cfg.prv_horz_info_sph = 2;
		data->vfmt->cfg.prv_horz_info_eph = (data->vfmt->pix.width +
						    data->vfmt->cfg.prv_horz_info_sph) - 1;
		data->vfmt->cfg.prv_vert_info_elv = data->vfmt->pix.height - 1;
	}
	...
	if (CONFIG_PRV_NF & data->flags) {
		...
		if (data->vfmt->cfg.prv_pcr_sdrport) {
			/* Add to-be-cropped bits to the input to keep output */
			data->vfmt->cfg.prv_horz_info_eph += 4;
			data->vfmt->cfg.prv_vert_info_elv += 4;
		}
	}
	...
	if (CONFIG_PRV_CFA & data->flags) {
		...
		if (data->vfmt->cfg.prv_pcr_sdrport) {
			/* Add to-be-cropped bits to the input to keep output */
			/* FIXME: Assume always CFA_FMT = Bayer */
			data->vfmt->cfg.prv_horz_info_eph += 4;
			data->vfmt->cfg.prv_vert_info_elv += 4;
		}
	}
	...
	if (data->vfmt->cfg.prv_pcr_sdrport) {
		val = SPH(data->vfmt->cfg.prv_horz_info_sph);
		val |= EPH(data->vfmt->cfg.prv_horz_info_eph);
		bits = SPH_MASK | EPH_MASK;
		omap_masked_writel(PRV_HORZ_INFO, val, bits);
		SPEW(2, "PRV_HORZ_INFO: SPH=%u EPH=%u\n",
			data->vfmt->cfg.prv_horz_info_sph,
			data->vfmt->cfg.prv_horz_info_eph);

		val = ELV(data->vfmt->cfg.prv_vert_info_elv);
		omap_masked_writel(PRV_VERT_INFO, val, ELV_MASK);
		SPEW(2, "PRV_VERT_INFO: ELV=%u\n", data->vfmt->cfg.prv_vert_info_elv);
	}
						...
						struct v4l2_pix_format *pix)
{
	...
	ctrl.id    = V4L2_CID_OMAP34XX_ISP_SMIA10_USE_DPCM;
	ctrl.value = vfmt->cfg.use_dpcm;
	...
	if (omap34xx_v4l2_int_master_ioctl_s_ctrl(&in->mst, &ctrl))
		goto exit;
