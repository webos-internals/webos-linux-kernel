...
*/
static BLOCKING_NOTIFIER_HEAD(timechange_notifier_list);

int timechange_notifier_register(struct notifier_block *nb)
{
	return blocking_notifier_chain_register(&timechange_notifier_list, nb);
}
EXPORT_SYMBOL(timechange_notifier_register);

int timechange_notifier_unregister(struct notifier_block *nb)
{
	return blocking_notifier_chain_unregister(&timechange_notifier_list, nb);
}
...
{
	blocking_notifier_call_chain(&timechange_notifier_list, 0, NULL);
}
