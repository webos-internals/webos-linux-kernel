...
struct tag_machinfo machinfo;
EXPORT_SYMBOL(machinfo);
#endif
...
#ifdef CONFIG_MACH_BRISKET
static int __init parse_tag_machinfo(const struct tag *tag)
{
	memcpy(&machinfo, &tag->u.mach, sizeof(struct tag_machinfo));

	printk(KERN_INFO "%s: machinfo triton version: Token = %02x, IDCODE = %02x\n",
	       __FUNCTION__,
	       machinfo.triton_version_token, machinfo.triton_version_idcode);
	
	return 0;
}
__tagtable(ATAG_MACHINFO, parse_tag_machinfo);
#endif
