...
static int __init omap_cpu_init(struct cpufreq_policy *policy)
{
	...
	/* we are already running at correct operating point,
	 * but because full initialization is only happening on opp transition
	 * we need to toggle current opp.
	 */
	constraint_set(vdd1_handle,
		get_vdd1_arm_constraint_for_freq(policy->min/1000));
	 
	constraint_set(vdd1_handle,
		get_vdd1_arm_constraint_for_freq(policy->cur/1000));
		
	constraint_register_pre_notification(vdd1_handle,
					&cpufreq_pre,  CO_VDD1_MAXLEVEL);
	constraint_register_post_notification(vdd1_handle,
					&cpufreq_post, CO_VDD1_MAXLEVEL);
