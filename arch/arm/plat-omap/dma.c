...
/**
 * @brief omap_dump_lch_reg : to dump dma registers
 * @param lch
 */
void omap_dump_lch_reg(int lch)
{
	printk(KERN_INFO" **************************"
	       " DMA Channel (%d) Registers"
	       "***************************\n", lch);
	printk(KERN_INFO"OMAP_DMA4_OCP_SYSCONFIG	: 0x%08x\n",
		OMAP_DMA4_OCP_SYSCONFIG);
	printk(KERN_INFO"OMAP_DMA4_IRQENABLE_L0 	: 0x%08x\n",
		OMAP_DMA4_IRQENABLE_L0);
	printk(KERN_INFO"OMAP_DMA4_IRQSTATUS_L0		: 0x%08x\n",
		OMAP_DMA4_IRQSTATUS_L0);
	printk(KERN_INFO"OMAP_DMA4_SYSSTATUS		: 0x%08x\n",
		OMAP_DMA4_SYSSTATUS);
	printk(KERN_INFO"OMAP_DMA4_GCR_REG		: 0x%08x\n",
		OMAP_DMA4_GCR_REG);

	printk(KERN_INFO"OMAP_DMA_CCR_REG(%d)		: 0x%08x\n", lch,
		OMAP_DMA_CCR_REG(lch));
	printk(KERN_INFO"OMAP_DMA_CLNK_CTRL_REG(%d)	: 0x%08x\n", lch,
		OMAP_DMA_CLNK_CTRL_REG(lch));
	printk(KERN_INFO"OMAP_DMA_CICR_REG(%d)		: 0x%08x\n", lch,
		OMAP_DMA_CICR_REG(lch));
	printk(KERN_INFO"OMAP_DMA_CSR_REG(%d)		: 0x%08x\n", lch,
		OMAP_DMA_CSR_REG(lch));
	printk(KERN_INFO"OMAP_DMA_CSDP_REG(%d)		: 0x%08x\n", lch,
		OMAP_DMA_CSDP_REG(lch));
	printk(KERN_INFO"OMAP_DMA_CEN_REG(%d)		: 0x%08x\n", lch,
		OMAP_DMA_CEN_REG(lch));
	printk(KERN_INFO"OMAP_DMA_CFN_REG(%d)		: 0x%08x\n", lch,
		OMAP_DMA_CFN_REG(lch));
	printk(KERN_INFO"OMAP2_DMA_CSSA_REG(%d)		: 0x%08x\n", lch,
		OMAP2_DMA_CSSA_REG(lch));
	printk(KERN_INFO"OMAP2_DMA_CDSA_REG(%d)		: 0x%08x\n", lch,
		OMAP2_DMA_CDSA_REG(lch));
	printk(KERN_INFO"OMAP_DMA_CSEI_REG(%d)		: 0x%08x\n", lch,
		OMAP_DMA_CSEI_REG(lch));
	printk(KERN_INFO"OMAP_DMA_CSFI_REG(%d)		: 0x%08x\n", lch,
		OMAP_DMA_CSFI_REG(lch));
	printk(KERN_INFO"OMAP_DMA_CDEI_REG(%d)		: 0x%08x\n", lch,
		OMAP_DMA_CDEI_REG(lch));
	printk(KERN_INFO"OMAP_DMA_CDFI_REG(%d)		: 0x%08x\n", lch,
		OMAP_DMA_CDFI_REG(lch));
	printk(KERN_INFO"OMAP_DMA_CSAC_REG(%d)		: 0x%08x\n", lch,
		OMAP_DMA_CSAC_REG(lch));
	printk(KERN_INFO"OMAP_DMA_CDAC_REG(%d)		: 0x%08x\n", lch,
		OMAP_DMA_CDAC_REG(lch));
	printk(KERN_INFO"OMAP2_DMA_CCEN_REG(%d)		: 0x%08x\n", lch,
		OMAP2_DMA_CCEN_REG(lch));
	printk(KERN_INFO"OMAP2_DMA_CCFN_REG(%d)		: 0x%08x\n", lch,
		OMAP2_DMA_CCFN_REG(lch));
	printk(KERN_INFO"OMAP2_DMA_COLOR_REG(%d)	: 0x%08x\n", lch,
		OMAP2_DMA_COLOR_REG(lch));
}
EXPORT_SYMBOL(omap_dump_lch_reg);
	...
	if (unlikely(status & OMAP2_DMA_TRANS_ERR_IRQ)) {
		...
		if (cpu_class_is_omap2()) {
			 ...
			 * after a transaction error. So we explicitely 
		 	 * disable the channel
		 	 */
			OMAP_DMA_CCR_REG(ch) &= ~OMAP_DMA_CCR_EN;
			dma_chan[ch].flags &= ~OMAP_DMA_ACTIVE;
		}
	}
...
static irqreturn_t omap2_dma_irq_handler(int irq, void *dev_id)
{
	...
	enblreg = omap_readl(OMAP_DMA4_IRQENABLE_L0);
	val &= enblreg; /* Dispatch only relevant interrupts */
