...
static void set_pinmux_range (u16 start, u16 end, u16 val)
{
	u16 addr;
...
int board_lvds_enable(int enable)
{
//	printk("####### %s:\n", __func__ );
	if ( enable ) 	{
		/* Mux all dss pins as MODE0 */
		set_pinmux_range ( 0x0d4, 0x010a, 0x0);
	}
	else {
		/* Mux all dss pins as MODE7+PD */
		set_pinmux_range ( 0x0d4, 0x010a, 0x10F );
	}
	return 0;
}
#endif
