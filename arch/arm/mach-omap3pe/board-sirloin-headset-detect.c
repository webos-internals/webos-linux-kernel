...
#define SIRLOIN_GPIO_HS_PLUG_IN_MUX_NORMAL	"B26_3430_GPIO111"

#ifdef CONFIG_MACH_SIRLOIN_3630

#  define SIRLOIN_GPIO_MIC_ANS_ON		"R1_3430_MIC_ANS_ON"
#  define SIRLOIN_GPIO_MIC_ANS_OFF		"R1_3430_MIC_ANS_OFF"

#  define SIRLOIN_GPIO_MIC_ANS_INT		109
#  define SIRLOIN_GPIO_MIC_ANS_INT_MUX_NORMAL	"B25_3430_GPIO_109"

#  define SIRLOIN_GPIO_HS_AMP_SD_N		110
#  define SIRLOIN_GPIO_HS_AMP_MUX_NORMAL	"C26_3430_GPIO_110"
#  define SIRLOIN_GPIO_HS_AMP_MUX_OFFMODE	"C26_3430_GPIO_110_OFFMODE"

#else   /* CONFIG_MACH_SIRLOIN_3630 */

#  define SIRLOIN_GPIO_MIC_ANS_ON		"Y4_3430_MIC_ANS_ON"
#  define SIRLOIN_GPIO_MIC_ANS_OFF		"Y4_3430_MIC_ANS_OFF"

#  define SIRLOIN_GPIO_MIC_ANS_INT		87
#  define SIRLOIN_GPIO_MIC_ANS_INT_MUX_NORMAL	"H27_3430_GPIO087"
...
static void headset_mic_detect_handler(struct delayed_work *work)
{
	...
	if ((dev->num_mic_check++ < MIC_CHECK_MAX) && dev->headset_inserted && !dev->mic_present) {
		...
		if (mic) {
			dev->mic_present = mic;
			omap_cfg_reg(SIRLOIN_GPIO_MIC_ANS_ON); /* MIC-ANS */
...
static void headset_insert_handler(struct headset_device *dev)
{
	...
	if (mic != dev->mic_present) {
		...
		if (mic) {
			...
			omap_cfg_reg(SIRLOIN_GPIO_MIC_ANS_OFF); /* MIC-ANS */
		}
	}
...
static int __init headset_probe(struct platform_device *pdev)
{
	...
	omap_cfg_reg(SIRLOIN_GPIO_HS_PLUG_IN_MUX_NORMAL);    /* HS-PLUG-IN  */
	omap_cfg_reg(SIRLOIN_GPIO_HS_AMP_MUX_NORMAL);        /* HS-AMP-SD~  */
	omap_cfg_reg(SIRLOIN_GPIO_MIC_ANS_OFF);              /* MIC-ANS     */
	omap_cfg_reg(SIRLOIN_GPIO_MIC_ANS_INT_MUX_NORMAL);   /* MIC-ANS-INT */
