...
#define MSM_CAMERA_FLASH_SRC_USER (0x00000001<<2)
...
struct msm_camera_sensor_flash_user {
	uint32_t low_current;
	uint32_t high_current;
	int (*set_current)(uint8_t is_flash_mode, uint32_t mA);
	int (*enable_flash)(uint8_t enable);
};
...
struct msm_camera_sensor_flash_src {
	...
	union {
		...
		struct msm_camera_sensor_flash_user user_src;
	} _fsrc;
};
...
enum msm_panel_rotation {
	MSM_PANEL_CONFIG_FLIP_VERTICAL = (1<<1),
	MSM_PANEL_CONFIG_FLIP_HORIZONTAL = (1<<2),
};
...
struct msm_i2c_platform_data {
	...
	int (*msm_i2c_clock_set_state)(int state);
