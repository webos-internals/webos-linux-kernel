...
#include <asm/arch/qdsp5/qdsp5audplaycmdi.h>
#include <asm/arch/qdsp5/qdsp5audplaymsg.h>
...
static void audaac_update_stream_info(struct audio *audio, uint32_t *payload)
{
	...
	spin_unlock_irqrestore(&audio->dsp_lock, flags);
	MM_DBG("audio_update_stream_info: chan_info=%d, sample_rate=%d,\
					bit_stream_info=%d\n",\
			   ...
			   size_t count, loff_t *pos)
{
	...
	MM_DBG("audio_write: eos_condition %x buf[0x%x] start[0x%x]\n",
			eos_condition, (unsigned int)buf, (unsigned int)start);
...
static int __init audio_init(void)
{
	...
	INIT_LIST_HEAD(&the_aac_audio.free_event_queue);
