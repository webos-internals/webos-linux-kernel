...
#define HS_WITH_MIC_INSERT	0x00
#define HS_WITHOUT_MIC_INSERT	0xFD
#define HS_WITHOUT_MIC_REMOVE	0xFE
#define HS_WITH_MIC_REMOVE	0xFF
#define HS_REMOVE	0xFF

#ifdef CONFIG_MACH_RIB
#define HS_DET_NEW_REPORTING_MAJOR 0
#define HS_DET_NEW_REPORTING_MINOR 4
#else
#define HS_DET_NEW_REPORTING_MAJOR 0xFF
#define HS_DET_NEW_REPORTING_MINOR 0xFF
#endif

#define SMEM_MODEM_VERSION_INFO 135
...
static const uint32_t hs_key_map[] = {
	...
	KEY(HS_HEADSET_SWITCH_K, KEY_PLAYPAUSE),
...
struct msm_handset {
	...
	unsigned mic_inserted;
	unsigned button_pressed;

	unsigned new_reporting;
};

static struct msm_rpc_client *rpc_client;
static struct msm_handset *hs;

static u32 headset_state = 0;
static u32 headset_mic_state = 0;

static ssize_t headset_state_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "headphone %d : headset with mic %d\n", headset_state, headset_mic_state);
}

static DEVICE_ATTR(headset_state, 0644, headset_state_show, NULL);
 ...
 *
 * Note: on shank/rib, modem will always send hs with mic first
 * after timeout, headset without mic will be sent if mic is not found
 * if using modem version 4.0 and higher, then do new sequence, otherwise use old one
 */
static void report_hs_key(uint32_t key_code, uint32_t key_parm)
{
	...
	if(key == KEY_PLAYPAUSE || key == SW_HEADPHONE_INSERT)
		printk ("Headset Key: %d code: %d param: %d\n", key, key_code, key_parm);
	...
	case SW_HEADPHONE_INSERT:
		report_headset_switch(hs->ipdev, key, \
			( (key_parm == HS_WITH_MIC_INSERT) || \
			(key_parm == HS_WITHOUT_MIC_INSERT) ) );
		// Generate the SW_MICROPHONE_INSERT as needed
		if ( (HS_WITH_MIC_INSERT == key_parm) || \
			(HS_WITH_MIC_REMOVE == key_parm) ) {
			if (HS_WITH_MIC_REMOVE == key_parm) {
				input_report_key(hs->ipdev, KEY_PLAYPAUSE, 0);
			}
			report_headset_switch(hs->ipdev, SW_MICROPHONE_INSERT, \
				 (key_parm == HS_WITH_MIC_INSERT) );
...
static void report_hs_key_new(uint32_t key_code, uint32_t key_parm)
{
	int key, temp_key_code;
	...
	case KEY_PLAYPAUSE:
		if (hs->mic_inserted) {
			input_report_key(hs->ipdev, key, (key_code != HS_REL_K));
			hs->button_pressed = (key_code != HS_REL_K);
			printk(KERN_INFO "Headset button %d\n", hs->button_pressed);
		}
		else
			printk(KERN_WARNING "Headset button pressed, but there is no mic!");
		break;
	case SW_HEADPHONE_INSERT:

		if (key_parm == HS_WITHOUT_MIC_INSERT) {
			hs->mic_inserted = 0;
			hs->button_pressed = 0;
		...
		else if (key_parm == HS_WITH_MIC_INSERT) {
			hs->mic_inserted = 1;
			report_headset_switch(hs->ipdev, SW_MICROPHONE_INSERT, 1);
			printk(KERN_INFO "Headset with mic INSERTED\n");
			headset_mic_state = 1;
		} else if (key_parm == HS_REMOVE) {
			if (hs->mic_inserted) {
				if (hs->button_pressed) {
					input_report_key(hs->ipdev, KEY_PLAYPAUSE, 0);
					printk(KERN_INFO "Headset button 0\n");
				}
			}
			hs->mic_inserted = 0;
			hs->button_pressed = 0;
			report_headset_switch(hs->ipdev, SW_MICROPHONE_INSERT, 0);
			   ...
			   struct rpc_request_hdr *req, unsigned len)
{
	...
	case RPC_KEYPAD_PASS_KEY_CODE_PROC: {
		...
		if (hs->new_reporting)
			report_hs_key_new(args->key_code, args->key_parm);
		else
...
static int process_subs_srvc_callback(struct hs_event_cb_recv *recv)
{
	...
	if (hs->new_reporting)
		report_hs_key_new(be32_to_cpu(recv->key.code), be32_to_cpu(recv->key.parm));
	else
...
/*
 * shared memory has modem version info, done similarly in boot loader
 */
struct palm_modeminfo_v1
{
	uint32_t format;
...
static int __devinit hs_probe(struct platform_device *pdev)
{
	...
	info = smem_alloc(135, sizeof(struct palm_modeminfo_v1));
	if (!info) {
		printk(KERN_INFO "Palm modem version not found!\n");
	} else {
		major = (info->v1.api_version & ~((1<<16)-1)) >> 16;
		minor = info->v1.api_version & ((1<<16)-1);

		if (major > HS_DET_NEW_REPORTING_MAJOR ||
			(major == HS_DET_NEW_REPORTING_MAJOR && minor >= HS_DET_NEW_REPORTING_MINOR)) {
			printk(KERN_INFO "modem version %d.%d supports new hs reporting\n", major, minor);
			hs->new_reporting = 1;
		} else
			printk(KERN_INFO "modem version %d.%d does not support new hs reporting\n", major, minor);
	}
	...
	if ( device_create_file(&pdev->dev, &dev_attr_headset_state) < 0)
		dev_err(&pdev->dev, "failed to add headset state sysfs\n");
