...
#include "keypad_ma87712_crespo.h"
#endif

#include "devices.h"
#include "devices-msm8x60.h"
#include "devices-crespo.h"
#include "cpuidle.h"
#include "pm.h"
#include "rpm.h"
#include "spm.h"
...
enum twain_board_types {
	...
	TWAIN_DVT,
	TWAIN_PVT,
	TWAIN_3G_PROTO = TWAIN_PROTO,
	TWAIN_3G_PROTO2 = TWAIN_PROTO2,
	TWAIN_3G_EVT1 = TWAIN_EVT1,
	TWAIN_3G_EVT2 = TWAIN_EVT2,
	TWAIN_3G_EVT3 = TWAIN_EVT3,
	TWAIN_3G_DVT = TWAIN_DVT,
	TWAIN_3G_PVT = TWAIN_PVT,
};

static u32 board_type = TWAIN_PROTO; /* In case we get a lazy bootloader */

static struct {
	enum twain_board_types type;
	const char *str;
} boardtype_tbl[] = {
	/* WiFi */
	{TWAIN_PROTO,    "twain-Wifi-proto"},
	{TWAIN_PROTO2,   "twain-Wifi-proto2"},
	{TWAIN_EVT1,     "twain-Wifi-evt1"},
	{TWAIN_EVT2,     "twain-Wifi-evt2"},
	{TWAIN_EVT3,     "twain-Wifi-evt3"},
	{TWAIN_DVT,      "twain-Wifi-dvt"},
	{TWAIN_PVT,      "twain-Wifi-pvt"},

	/* 3G */
	{TWAIN_3G_PROTO,    "twain-3G-proto"},
	{TWAIN_3G_PROTO2,   "twain-3G-proto2"},
	{TWAIN_3G_EVT1,     "twain-3G-evt1"},
	{TWAIN_3G_EVT2,     "twain-3G-evt2"},
	{TWAIN_3G_EVT3,     "twain-3G-evt3"},
	{TWAIN_3G_DVT,      "twain-3G-dvt"},
	{TWAIN_3G_PVT,      "twain-3G-pvt"},

	/* TODO: Non-standard board strings, to be removed once all copies of
	 * bootie in the wild are updated to use the above format */

	/* WiFi */
	{TWAIN_PROTO,    "twain-1stbuild-Wifi"},
	{TWAIN_PROTO2,   "twain-2ndbuild-Wifi"},
	{TWAIN_EVT1,     "twain-3rdbuild-Wifi"},
	{TWAIN_EVT2,     "twain-4thbuild-Wifi"},
	{TWAIN_EVT3,     "twain-5thbuild-Wifi"},
	{TWAIN_DVT,      "twain-6thbuild-Wifi"},
	{TWAIN_PVT,      "twain-7thbuild-Wifi"},
	{TWAIN_PVT,      "twain-pvt-Wifi"},

	/* 3G */
	{TWAIN_3G_PROTO,    "twain-1stbuild-3G"},
	{TWAIN_3G_PROTO2,   "twain-2ndbuild-3G"},
	{TWAIN_3G_EVT1,     "twain-3rdbuild-3G"},
	{TWAIN_3G_EVT2,     "twain-4thbuild-3G"},
	{TWAIN_3G_EVT3,     "twain-5thbuild-3G"},
	{TWAIN_3G_DVT,      "twain-6thbuild-3G"},
	{TWAIN_3G_PVT,      "twain-7thbuild-3G"},
	{TWAIN_3G_PVT,      "twain-pvt-3G"}
};
...
static u32 board_lcd_type = LCD_LG;
...
static int isp1763_modem_gpio_init(void)
{
	...
	gpio_pwr_3g_en = get_3g_3v3_enable_gpio();
	...
	if (rc < 0) {
		pr_err("%s: GPIO_3G_UIM_CD_N gpio %d request failed\n", __func__, GPIO_3G_UIM_CD_N);
	}
...
static int msm_hsusb_pmic_id_notif_init(void (*callback)(int online), int init)
{
	...
	if (machine_is_msm8x60_ffa() || machine_is_crespo()) {
...
/*
CRESPO_A6_0_TCK			115
CRESPO_A6_0_WAKEUP		78
CRESPO_A6_0_TDIO		116
CRESPO_A6_0_MSM_IRQ		94
*/
...
static struct a6_platform_data crespo_a6_0_platform_data = {
	.dev_name			= A6_DEVICE_0,
	.pwr_gpio			= CRESPO_A6_0_MSM_IRQ,
	.sbw_tck_gpio			= CRESPO_A6_0_TCK,
	.sbw_tdio_gpio			= CRESPO_A6_0_TDIO,
	.sbw_wkup_gpio			= CRESPO_A6_0_WAKEUP,
...
static struct i2c_board_info a6_0_i2c_board_info = {
	I2C_BOARD_INFO( A6_DEVICE_0, (0x64>>1)),
	.platform_data = NULL,
};
...
static void __init crespo_init_a6(void)
{
	// Fixup A6 platform data
	crespo_a6_0_platform_data.pwr_gpio_wakeup_cap =
		msm_mpm_is_app_irq_wakeup_capable(gpio_to_irq(crespo_a6_0_platform_data.pwr_gpio));

	printk(KERN_ERR "Registering a6_0 device.\n");
	a6_0_i2c_board_info.platform_data = &crespo_a6_0_platform_data;
	i2c_register_board_info(MSM_GSBI8_QUP_I2C_BUS_ID, &a6_0_i2c_board_info, 1);
	a6_sbw_init_imp(&crespo_a6_0_platform_data);
...
bool gpios_vreg_crespo_cam_on = false;
static int crespo_camera_gpios[] = {
	CRESPO_CAM_I2C_DATA,
	CRESPO_CAM_I2C_CLK,
	CRESPO_CAMIF_MCLK,
};
...
static int config_camera_on_gpios_web_cam_mt9m114(void)
{
	int rc = 0;
	...
	CDBG("+++ %s\n", __func__);
	if ( !gpios_vreg_crespo_cam_on ) {

		configure_gpiomux_gpios(1, crespo_camera_gpios,
			ARRAY_SIZE(crespo_camera_gpios));
...
err:
	configure_gpiomux_gpios(0, crespo_camera_gpios,
			ARRAY_SIZE(crespo_camera_gpios));
...
static void config_camera_off_gpios_web_cam_mt9m114(void)
{
	int rc=0;
	struct pm8058_gpio_cfg {
		int    gpio;
	...
	struct pm8058_gpio_cfg camera_reset_cfg={
			/* PM8058 GPIO_08 Twain 1.3M Camera Reset Pin*/
			7,
			{
				.direction	= PM_GPIO_DIR_OUT,
				.output_value	= 1,
	...
	if (gpios_vreg_crespo_cam_on) {
		...
		gpios_vreg_crespo_cam_on = false;
	}

	CDBG("--- %s\n", __func__);
}

struct msm_camera_device_platform_data msm_camera_device_data_web_cam_mt9m114 = {
	.camera_gpio_on  = config_camera_on_gpios_web_cam_mt9m114,
	.camera_gpio_off = config_camera_off_gpios_web_cam_mt9m114,
...
static struct msm_camera_sensor_info msm_camera_sensor_mt9m114_data = {
	...
	.sensor_pwd		= NONE, //No use on mt9m114 module
	.vcm_pwd		= NONE, //No use on crespo.
	.vcm_enable		= 0,
	.pdata			= &msm_camera_device_data_web_cam_mt9m114,
...
static struct i2c_board_info msm_camera_boardinfo[] __initdata = {
	#ifdef CONFIG_WEBCAM_MT9M114
	{
		//Mt9m114 I2C address depends on SADDR pin, if low, then r/w address are 0x91/0x90,
		//if SADDR pin is high, then r/w address should be 0xBB/0xBA.
		//For the module we selected, 8-bit address is 0xBA. 
		I2C_BOARD_INFO("mt9m114", 0xBA>>1),
	},
	#endif
};
#endif
...
static struct regulator *vreg_s3a = NULL;
static struct regulator *vreg_l2a = NULL;

static int ma87712_set_power(int on)
{
	pr_info("%s: enter\n", __func__);

	if (vreg_s3a == NULL)
	{
		vreg_s3a = regulator_get(NULL, "8058_s3");
		if (vreg_s3a == NULL)
		{
			pr_err("%s: Unable to get 8058_s3 regulator\n", __func__);
			goto err;
		}
	}
	...
	if (vreg_l2a == NULL)
	{
		...
		if (vreg_l2a == NULL)
		{
			pr_err("%s: Unable to get 8058_l2 regulator\n", __func__);
			goto err;
		}
	}
	...
	{
		// power up
		if (regulator_set_voltage(vreg_s3a, 1800000, 1800000))
		{
			pr_err("%s: Unable to set regulator 8058_s3 voltage\n", __func__);
			goto err;
		}
		if (regulator_enable(vreg_s3a))
		{
			pr_err("%s: Unable to enable regulator 8058_s3\n", __func__);
			goto err;
		}

		if (regulator_set_voltage(vreg_l2a, 2600000,2600000))
		{
			pr_err("%s: Unable to set regulator 8058_l2 voltage\n", __func__);
			goto err;
		}
		if (regulator_enable(vreg_l2a))
		{
			pr_err("%s: Unable to enable regulator 8058_l2\n", __func__);
			goto err;
		}
	...
	{
		// power down
		regulator_disable(vreg_s3a);
		regulator_put(vreg_s3a);

		regulator_disable(vreg_l2a);
		regulator_put(vreg_l2a);
...
static struct ma87712_platform_data  ma87712_pdata = {
	.set_power = ma87712_set_power,
	.gpio_rst_n = MA87712_KBC_RST_N,
	.gpio_pd_n = MA87712_KBC_PD_N,
	.gpio_int_n = MA87712_KBC_INT,
	.keycode = KEYPAD_MA87712_KEYCODE,
	.keycodemax = KEYPAD_MA87712_KEYCODEMAX,
	.keycodesize = KEYPAD_MA87712_KEYCODESIZE,
};

static struct i2c_board_info ma87712_i2c_info[] = {
	{
		I2C_BOARD_INFO(MA87712_DRIVER, MA87712_SLAVE_ADDR),	//slave address 0x73
		.irq = MSM_GPIO_TO_INT(MA87712_KBC_INT),
		.platform_data = &ma87712_pdata,
	},
};

#endif // if defined(CONFIG_KEYBOARD_MA87712) || defined(CONFIG_KEYBOARD_MA87712_MODULE)
...
{
	return gpio_get_value(GPIO_CTP_INT_3G);
}
...
static struct bluetooth_power_state_platform_data bt_power_state_crespo = {
	.dev_name = "bt_power",
	.bt_power = bt_power
};
...
static struct platform_device bluetooth_power_state_device = {
	...
	.id = 0,
	.dev = {
		.platform_data = &bt_power_state_crespo
	}
};
...
static int __init board_gsbi_init(int gsbi, u32 prot)
{
	int rc;
	u32 gsbi_phys;
	void *gsbi_virt;

	pr_debug("%s: gsbi=%d prot=%x\n", __func__, gsbi, prot);
...
static int __init board_gsbi5_init(void)
{
	return (board_gsbi_init(5, I2C_ON_2_PORTS_UART));
}
...
static struct wm8994_pdata wm8958_pdata = {
        ...
        .micdet_irq = CRESPO_AUD_HEAD_MIC_DET_IRQ_GPIO,
...
static void __init msm8x60_init_ebi2(void)
{
	...
	if (ebi2_cfg_ptr != 0) {
		...
		if (machine_is_msm8x60_surf() || machine_is_msm8x60_ffa() || machine_is_crespo())
...
static uint32_t crespo_tlmm_cfgs[] = {
...
static void __init msm8x60_init_tlmm(void)
{
	...
	for (n = 0; n < ARRAY_SIZE(crespo_tlmm_cfgs); ++n)
		gpio_tlmm_config(crespo_tlmm_cfgs[n], 0);
}
...
static int crespo_wifi_power(int on)
{
	...
	int gpios[] = {
			CRESPO_GPIO_WL_HOST_WAKE,
			CRESPO_GPIO_HOST_WAKE_WL,
			CRESPO_GPIO_WLAN_RST_N,
		};
	...
	if (on) {
		...
		gpio_direction_output(CRESPO_GPIO_WLAN_RST_N, 0);

		mdelay(5);

		gpio_direction_output(CRESPO_GPIO_WLAN_RST_N, 1);
	}
	else
	{
		//CHIP_PWD -> wait (Tc 5)
...
static void crespo_probe_wifi(int id, struct mmc_host *mmc)
{
	printk("%s: id %d mmc %p\n", __PRETTY_FUNCTION__, id, mmc);
	wifi_mmc = mmc;
}

static void crespo_remove_wifi(int id, struct mmc_host *mmc)
{
	printk("%s: id %d mmc %p\n", __PRETTY_FUNCTION__, id, mmc);
	wifi_mmc = NULL;
}
...
static struct mmc_platform_data msm8x60_sdc4_data = {
	...
	.translate_vdd  = msm_sdcc_setup_power,
	.mmc_bus_width  = MMC_CAP_4_BIT_DATA,
	.board_probe	= crespo_probe_wifi,
	.board_remove	= crespo_remove_wifi,
	.dummy52_required = 1,
	.msmsdcc_fmin	= 400000,
	.msmsdcc_fmid	= 24000000,
	.msmsdcc_fmax	= 48000000,
...
#define CRESPO_CLOCK_FIXUP 1
#if CRESPO_CLOCK_FIXUP
...
static struct msm_board_data crespo_board_data __initdata = {
	.gpiomux_cfgs = crespo_gpiomux_cfgs,
};
...
static void __init msm8x60_init(struct msm_board_data *board_data)
{
	...
	platform_add_devices(crespo_devices,
					ARRAY_SIZE(crespo_devices));
	register_lcdc_panel();
...
static void __init crespo_init(void)
{

	msm8x60_init(&crespo_board_data);
}
