...
/*
 * Dump all the pending replies for incoming calls
 */
int msm_rpcrouter_dump_pending_replies(void)
{
	int count = 0;
	struct msm_rpc_reply *reply = 0;
	struct msm_rpc_endpoint *ept = 0;
	list_for_each_entry(ept, &local_endpoints, list) {
		list_for_each_entry(reply, &ept->reply_pend_q, list) {
			printk(KERN_INFO"%s: ept %p pending reply for xid %x prog %x\n",
				 __func__,(void *)ept, reply->xid, reply->prog);
			count++;
		}
	}

	printk(KERN_INFO"%s: total pending replies: %d\n", __func__,count);
...
static int dump_pending_replies(char *buf, int max)
{
	int i = 0;
	int count = 0;
	unsigned long flags;
	...
	list_for_each_entry(ept, &local_endpoints, list) {
		...
		i += scnprintf(buf + i, max - i, "For ept: %p\n",(void *)ept);

		list_for_each_entry(reply, &ept->reply_pend_q, list) {
			i += scnprintf(buf + i, max - i, "pid: 0x%08x prog: 0x%08x xid 0x%08x\n",
				reply->pid, reply->prog, reply->xid);
			count++;
		}
	}

	i += scnprintf(buf + i, max - i, "Total pending replies: %d\n",count);
...
static void debugfs_init(void)
{
	...
	debug_create("dump_servers", 0444, dent,
		     dump_servers);
	debug_create("dump_pending_replies", 0444, dent,
		     dump_pending_replies);
