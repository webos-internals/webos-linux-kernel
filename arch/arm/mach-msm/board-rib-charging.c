		...
		const char *buf, size_t n)
{
	if (strncmp(buf, "off", 3) == 0 || strncmp(buf, "none", 4) == 0) {
		...
		maxMA = getMA(buf+4, 560);

		/* Enforcing a imax limit to prevent drawing
		   excessive current from the puck */
		if(maxMA > IMAX_LIMIT)
			maxMA = IMAX_LIMIT;

		puck_charging();
	}
	else if (strncmp(buf, "holster", 3) == 0) {
		maxMA = getMA(buf+3, 560);
		holster_charging();
	}
	else {
