...
#ifdef CONFIG_WAKE_SOURCES
	if (!from_idle && !fastpath_fastsleep_get())
		clear_wakeup_events();
#endif
...
static void boot_wall_put_args(char *cmd)
{
	/* TODO machine_restart can invoke boot_wall_put_args with NULL for a power wiggle-wiggle...
	 * This needs to be fixed in the hardkey reboot code to return a reasonable
	 * reboot reason. In the meantime, just say reboot.
	 */

	if (NULL == cmd) {
		if (restart_reason != 0x776655AA) { //default 
			goto done;
		}
	...
	} else if (!strcmp(cmd, "shutdown")) {  // Palm specific,
	...
	printk(KERN_WARNING "wall message: %s, restart_reason: 0x%x\n", cmd?cmd:"<NULL>", restart_reason);
}
