...
static int gpio_show_set(void *data, u64 gpio)
{
	unsigned i;
	unsigned v;

	for (i = 0; i < ARRAY_SIZE(msm_gpio_chips); i++) {
		if ((gpio >= msm_gpio_chips[i].chip.start)
			&& (gpio <= msm_gpio_chips[i].chip.end))
			break;
	}

	if (i == ARRAY_SIZE(msm_gpio_chips))
		return (-EINVAL);

	pr_info("GPIO:         %llu\n", gpio);

	v = readl(msm_gpio_chips[i].regs.out);
	pr_info("OUT:          %u\n", (v >> (gpio - msm_gpio_chips[i].chip.start)) & 0x1);

	v = readl(msm_gpio_chips[i].regs.oe);
	pr_info("OE:           %u\n", (v >> (gpio - msm_gpio_chips[i].chip.start)) & 0x1);

	writel(gpio, msm_gpio_chips[i].regs.page);
	v = readl(msm_gpio_chips[i].regs.cfg);
	pr_info("CFG:          %03x\n", v);

	v = readl(msm_gpio_chips[i].regs.in);
	pr_info("IN:           %u\n", (v >> (gpio - msm_gpio_chips[i].chip.start)) & 0x1);

	v = readl(msm_gpio_chips[i].regs.int_edge);
	pr_info("INT_DETECT:   %u\n", (v >> (gpio - msm_gpio_chips[i].chip.start)) & 0x1);

	v = readl(msm_gpio_chips[i].regs.int_pos);
	pr_info("INT_POLARITY: %u\n", (v >> (gpio - msm_gpio_chips[i].chip.start)) & 0x1);

	v = readl(msm_gpio_chips[i].regs.int_en);
	pr_info("INT_EN:       %u\n", (v >> (gpio - msm_gpio_chips[i].chip.start)) & 0x1);

	v = readl(msm_gpio_chips[i].regs.int_status);
	pr_info("INT_STATUS:   %u\n", (v >> (gpio - msm_gpio_chips[i].chip.start)) & 0x1);

	return (0);
}

DEFINE_SIMPLE_ATTRIBUTE(gpio_show_fops, NULL, gpio_show_set, "%llu\n");
...
static int __init gpio_debug_init(void)
{
	...
	debugfs_create_file("show", 0400, dent, 0, &gpio_show_fops);
	return 0;
}
