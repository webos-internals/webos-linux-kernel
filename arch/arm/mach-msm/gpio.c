...
struct gpio_state
{
	uint32_t out[ARRAY_SIZE(msm_gpio_chips)];
	uint32_t oe[ARRAY_SIZE(msm_gpio_chips)];
	uint32_t int_edge[ARRAY_SIZE(msm_gpio_chips)];
	uint32_t int_pos[ARRAY_SIZE(msm_gpio_chips)];
	uint32_t int_en[ARRAY_SIZE(msm_gpio_chips)];
};

static struct gpio_state store_gpios;
...
void msm_gpio_store(void)
{
	int i;

	for(i=0; i < ARRAY_SIZE(msm_gpio_chips); i++)
	{
		store_gpios.out[i] = readl(msm_gpio_chips[i].regs.out);
		store_gpios.oe[i] = readl(msm_gpio_chips[i].regs.oe);
		store_gpios.int_edge[i] = readl(msm_gpio_chips[i].regs.int_edge);
		store_gpios.int_pos[i] = readl(msm_gpio_chips[i].regs.int_pos);
		store_gpios.int_en[i] = readl(msm_gpio_chips[i].regs.int_en);

		if (msm_gpio_debug_mask & GPIO_DEBUG_SLEEP)
			printk("Chip[%d]: out=0x%x, oe=0x%x, edge=0x%x, pos=0x%x, en=0x%x\n", i, 
				store_gpios.out[i],
				store_gpios.oe[i],
				store_gpios.int_edge[i],
				store_gpios.int_pos[i],
				store_gpios.int_en[i]);
	}
}

void msm_gpio_dump(void)
{
	int i;

	for(i=0; i < ARRAY_SIZE(msm_gpio_chips); i++)
	{
		printk("Chip[%d]: in=0x%x, out=0x%x, oe=0x%x, edge=0x%x, pos=0x%x, en=0x%x\n", i,
...
void msm_gpio_restore(void)
{
	int i;

	for(i=0; i < ARRAY_SIZE(msm_gpio_chips); i++)
	{
		writel(store_gpios.out[i], msm_gpio_chips[i].regs.out);
		writel(store_gpios.oe[i], msm_gpio_chips[i].regs.oe);
		writel(store_gpios.int_edge[i], msm_gpio_chips[i].regs.int_edge);
		writel(store_gpios.int_pos[i], msm_gpio_chips[i].regs.int_pos);
		writel(store_gpios.int_en[i], msm_gpio_chips[i].regs.int_en);
...
static void gpio_enable_set(void *data, u64 val)
{
	msm_gpio_debug_result = gpio_tlmm_config(val, 0);
}
static void gpio_disable_set(void *data, u64 val)
{
	msm_gpio_debug_result = gpio_tlmm_config(val, 1);
}

static u64 gpio_debug_get(void *data)
{
