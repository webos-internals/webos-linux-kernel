...
#include <linux/a6.h>
#include "a6_sbw_impl_shortloin.h"
#endif
...
static int lcdc_hitachi_pmic_bl(int bl_level);
...
static u32 board_type = OPAL_PROTO; /* In case we get a lazy bootloader */
...
static struct {
	...
	{OPAL_DVT1,     "opal-Wifi-dvt1"},
	{OPAL_DVT2,     "opal-Wifi-dvt2"},
	{OPAL_PVT,      "opal-Wifi-pvt"},

	/* 3G */
	{OPAL_3G_PROTO,    "opal-3G-proto"},
	{OPAL_3G_PROTO2,   "opal-3G-proto2"},
	{OPAL_3G_EVT1,     "opal-3G-evt1"},
	{OPAL_3G_EVT2,     "opal-3G-evt2"},
	{OPAL_3G_EVT3,     "opal-3G-evt3"},
	{OPAL_3G_DVT1,     "opal-3G-dvt1"},
	{OPAL_3G_DVT2,     "opal-3G-dvt2"},
	{OPAL_3G_PVT,      "opal-3G-pvt"},
	...
	{OPAL_DVT1,     "opal-6thbuild-Wifi"},
	{OPAL_DVT2,     "opal-7thbuild-Wifi"},
	{OPAL_PVT,      "opal-pvt-Wifi"},

	/* 3G */
	{OPAL_3G_PROTO,    "opal-1stbuild-3G"},
	{OPAL_3G_PROTO2,   "opal-2ndbuild-3G"},
	{OPAL_3G_EVT1,     "opal-3rdbuild-3G"},
	{OPAL_3G_EVT2,     "opal-4thbuild-3G"},
	{OPAL_3G_EVT3,     "opal-5thbuild-3G"},
	{OPAL_3G_DVT1,     "opal-6thbuild-3G"},
	{OPAL_3G_DVT2,     "opal-7thbuild-3G"},
	{OPAL_3G_PVT,      "opal-pvt-3G"}
};
...
static int board_is_lite = 0;
static int __init board_lite_setup(char *lite_str)
{
	if (!strcmp(lite_str, "true"))
		board_is_lite = 1;

	return 0;
}
__setup("lite=", board_lite_setup);

// Pointer to wifi/3G pin arrays
static int *pin_table = NULL;

static u8 boardtype_is_3g(void) {
	if (board_type >= OPAL_3G_PROTO) {
		return 1;
	}
	else {
		return 0;
...
#define LCD_TYPE_IN_KERNEL_COMMAND_LINE 1
#ifdef LCD_TYPE_IN_KERNEL_COMMAND_LINE
enum board_lcd_types {
	LCD_HITACHI = 0,
	LCD_LG,
};

static u32 board_lcd_type = LCD_HITACHI;

static int __init board_lcd_type_setup(char *board_lcd_type_str)
{
	/* All we care about is LG */
	if (!strcmp(board_lcd_type_str, "lg"))
		board_lcd_type = LCD_LG;

	return 0;
}
__setup("lcd=", board_lcd_type_setup);
#endif
...
static int get_3g_3v3_enable_gpio(void)
{
	if (board_type == OPAL_3G_PROTO)
		return GPIO_3G_3V3_EN_PROTO;
	else if (board_type < OPAL_3G_EVT3)
		return GPIO_3G_3V3_EN_PRE_EVT3;
	else
		return GPIO_3G_3V3_EN;
}
...
static int isp1763_modem_gpio_init(int on)
{
	...
	int gpio_pwr_3g_en = get_3g_3v3_enable_gpio();
...
static struct platform_device brcm4751_device = {
	.name = "brcm4751",
	.id = -1,
	.dev = {
...
static struct msm_serial_hslite_platform_data msm_device_uart_dm4_pdata = {
	.config_gpio = 1,
	.uart_tx_gpio = BRCM4751_UART_TX_GPIO,
	.uart_rx_gpio = BRCM4751_UART_RX_GPIO,
	.uart_rfr_gpio = BRCM4751_UART_RFR_GPIO,
	.uart_cts_gpio = BRCM4751_UART_CTS_GPIO,
	.config_setting	= 1,
	.gsbi_protocol	= 0x40, //GSBI_PROTOCOL_UART
	.gsbi_fifosize	= 64,
};
#endif
...
static int msm_hsusb_pmic_id_notif_init(void (*callback)(int online), int init)
{
	...
	if (machine_is_msm8x60_ffa() || machine_is_shortloin()) {
...
void max8903b_set_DC_CHG_Mode_current(enum max8903b_current value)
{
	switch(value)	{
		case CURRENT_750MA:
			gpio_set_value(MAX8903B_GPIO_CHG_D_ISET_1,0); /* wall 750ma */
			gpio_set_value(MAX8903B_GPIO_CHG_D_ISET_2,0); /* wall 750ma */
			break;
		case CURRENT_900MA:
			gpio_set_value(MAX8903B_GPIO_CHG_D_ISET_1,0); /* wall 900ma */
			gpio_set_value(MAX8903B_GPIO_CHG_D_ISET_2,1); /* wall 900ma */
			break;
		case CURRENT_1400MA:
			gpio_set_value(MAX8903B_GPIO_CHG_D_ISET_1,1); /* wall 1400ma */
			gpio_set_value(MAX8903B_GPIO_CHG_D_ISET_2,1); /* wall 1400ma */
			break;
		case CURRENT_2000MA:
			gpio_set_value(MAX8903B_GPIO_CHG_D_ISET_1,1); /* wall 2000ma */
			gpio_set_value(MAX8903B_GPIO_CHG_D_ISET_2,0); /* wall 2000ma */
			break;
		default:
...
static void max8903b_suspend_gpio_config(void);
...
static struct a6_platform_data shortloin_a6_0_platform_data = {
	.dev_name			= A6_DEVICE_0,
	.pwr_gpio			= SHORTLOIN_A6_0_MSM_IRQ,
	.sbw_tck_gpio			= SHORTLOIN_A6_0_TCK,
	.sbw_tdio_gpio			= SHORTLOIN_A6_0_TDIO,
	.sbw_wkup_gpio			= SHORTLOIN_A6_0_WAKEUP,
...
static struct a6_platform_data shortloin_a6_1_platform_data = {
	.dev_name			= A6_DEVICE_1,
	.pwr_gpio       		= SHORTLOIN_A6_1_MSM_IRQ,
	.sbw_tck_gpio			= SHORTLOIN_A6_1_TCK,
	.sbw_tdio_gpio			= SHORTLOIN_A6_1_TDIO,
	.sbw_wkup_gpio			= SHORTLOIN_A6_1_WAKEUP,
...
static int a6_0_config_data[] = {
	SHORTLOIN_A6_0_MSM_IRQ,
};

static int a6_1_config_data[] = {
	SHORTLOIN_A6_1_MSM_IRQ,
};
...
static void __init shortloin_init_a6(void)
{
	// Fixup A6 platform data
	shortloin_a6_0_platform_data.pwr_gpio_wakeup_cap =
		msm_mpm_is_app_irq_wakeup_capable(gpio_to_irq(shortloin_a6_0_platform_data.pwr_gpio));

	shortloin_a6_1_platform_data.pwr_gpio_wakeup_cap =
		msm_mpm_is_app_irq_wakeup_capable(gpio_to_irq(shortloin_a6_1_platform_data.pwr_gpio));

	printk(KERN_ERR "Registering a6_0 device.\n");
	a6_0_i2c_board_info.platform_data = &shortloin_a6_0_platform_data;
	i2c_register_board_info(MSM_GSBI8_QUP_I2C_BUS_ID, &a6_0_i2c_board_info, 1);
	a6_sbw_init_imp(&shortloin_a6_0_platform_data);

	printk(KERN_ERR "Registering a6_1 device.\n");
	a6_1_i2c_board_info.platform_data = &shortloin_a6_1_platform_data;
	if (!board_is_lite) {
		i2c_register_board_info(MSM_GSBI8_QUP_I2C_BUS_ID, &a6_1_i2c_board_info, 1);
	}
	a6_sbw_init_imp(&shortloin_a6_1_platform_data);
...
bool gpios_vreg_shortloin_cam_on = false;
static int shortloin_camera_gpios[] = {
	SHORTLOIN_CAM_I2C_DATA,
	SHORTLOIN_CAM_I2C_CLK,
	SHORTLOIN_CAMIF_MCLK,
};

#define CAM_MCLK_FREQ 24000000

/* Cofigure vregs for cameras */
static int config_camera_vregs(bool on)
{
	int rc = 0;

	CDBG("CAMERA - config_camera_vreg power status:%d\n", on);

	if (on) {
		votg_lvs0 = regulator_get(NULL, "8058_lvs0");
		if (IS_ERR_OR_NULL(votg_lvs0)) {
			printk(KERN_ERR "%s: unable to get"
				" votg_lvs0\n", __func__);
			rc = -1;
			goto err;
		}
		...
		if (regulator_enable(votg_lvs0)) {
			printk(KERN_ERR "%s:Unable to enable"
				" votg_lvs0\n", __func__);
			rc = -1;
			goto err1;
		} else {
		...
		if (regulator_set_voltage(votg_vreg_l11, 2850000, 2850000)) {
			printk(KERN_ERR "%s: Unable to set regulator voltage:"
			" votg_l11\n", __func__);
			rc = -1;
			goto err2;
		}
		...
		if (regulator_enable(votg_vreg_l11)) {
			...
			CDBG("%s:enable votg_vreg_l11 succeed\n", __func__);
		}
	} else {
		if (IS_ERR_OR_NULL(votg_lvs0)) {
			CDBG("%s: votg_lvs0 is already off\n", __func__);
		} else {

			if (regulator_disable(votg_lvs0)) {
				printk(KERN_ERR "%s:Unable to disable"
					" votg_lvs0\n", __func__);
				rc = -1;
				goto err;
			} else {
				CDBG("%s:disable votg_lvs0"
					" succeed\n", __func__);
			}
		...
		if (IS_ERR_OR_NULL(votg_vreg_l11)) {
			CDBG("%s: votg_vreg_l11 is already off\n", __func__);
		} else {
...
/* Configure the common gpio for cameras */
static void config_camera_gpios(bool on)
{
	CDBG("CAMERA - configure gpios\n");
	if (on) {
		configure_gpiomux_gpios(1, shortloin_camera_gpios,
			ARRAY_SIZE(shortloin_camera_gpios));

	} else {
		configure_gpiomux_gpios(0, shortloin_camera_gpios,
			ARRAY_SIZE(shortloin_camera_gpios));
...
/* Configure MCLK for cameras */
static void config_camera_mclk(bool on)
{
	struct clk *clk = NULL;
...
/* Configure reset pin for mt9m113 */
static void mt9m113_config_reset(bool on)
{
	int rc = 0;

	if (on) {
		gpio_set_value_cansleep(SHORTLOIN_WEBCAM_RESET, 1);
		mdelay(20);
		gpio_set_value_cansleep(SHORTLOIN_WEBCAM_RESET, 0);
		mdelay(20);
		gpio_set_value_cansleep(SHORTLOIN_WEBCAM_RESET, 1);
		mdelay(20);
	} else {
...
static int config_camera_on_gpios_web_cam_mt9m113(void)
{
	...
	CDBG("+++ %s\n", __func__);
	if (!gpios_vreg_shortloin_cam_on) {

		config_camera_gpios(true);

		/* As camera power is always on, need Emulate power off/on
		sequence here to resolve some camera can't preview issue */
		rc = config_camera_vregs(false);
		if (rc < 0) {
			printk(KERN_ERR "%s config_camera_off_vreg"
				" failed", __func__);
			goto err;
		}
		...
		rc = config_camera_vregs(true);
		if (rc < 0) {
			printk(KERN_ERR "%s config_camera_on_vreg"
				" failed", __func__);
			goto err;
		}

		mt9m113_config_reset(true);

		config_camera_mclk(true);

		gpios_vreg_shortloin_cam_on = true;
	}
...
static void config_camera_off_gpios_web_cam_mt9m113(void)
{
	...
	if (gpios_vreg_shortloin_cam_on) {
		...
		/* Here keep camera vregs on when camera is off,
		    and use standby pin for camera power management. */

		config_camera_mclk(false);

		config_camera_gpios(false);

		mt9m113_config_reset(false);

		gpios_vreg_shortloin_cam_on = false;
	}
...
static struct msm_camera_sensor_info msm_camera_sensor_mt9m113_data = {
	.sensor_name	= "mt9m113",
	.sensor_reset	= 8, //PM8058 GPIO_8
	.sensor_pwd		= 107,
	.vcm_pwd		= NONE,//No use on shortloin.
	.vcm_enable		= 0,
...
/* Configure pwdn pin for vx6953 */
static void vx6953_config_pwdn(bool on)
{
	int rc = 0;

	if (on) {
		gpio_set_value_cansleep(SHORTLOIN_MAINCAM_PWDN, 1);
	} else {
		gpio_set_value_cansleep(SHORTLOIN_MAINCAM_PWDN, 0);
	}
...
static void config_camera_off_gpios_vx6953(void)
{
	...
	if (gpios_vreg_shortloin_cam_on) {
		...
		/* Here keep camera power on when camera is off,
		 and use pwdn pin for camera power management. */

		config_camera_mclk(false);

		config_camera_gpios(false);

		vx6953_config_pwdn(false);
...
int32_t gpio_enable_flash(uint8_t enable)
{
	if(boardtype_is_3g())
	{
		//For Opal_3g, need distinguish hardware version to select correct gpio for CAM_FLASH pin.
		if(board_type < OPAL_3G_EVT2)
		{
			gpio_set_value( SHORTLOIN_WEBCAM_FLASH_EVT1, enable );
		}
		else
		{
			gpio_set_value( SHORTLOIN_WEBCAM_FLASH, enable );
		}
	}
	else
	{
		//For Opal_wifi, which started from evt3, no need to distinguish hardware version.
		gpio_set_value( SHORTLOIN_WEBCAM_FLASH, enable );
	}
...
static struct msm_camera_sensor_flash_src msm_flash_src = {
	...
	._fsrc.user_src.enable_flash = gpio_enable_flash,
};

static struct msm_camera_sensor_flash_data msm_flash_vx6953 = {
...
static struct msm_camera_sensor_info msm_camera_sensor_vx6953_data = {
	...
	.resource		= msm_camera_resources,
	.num_resources	= ARRAY_SIZE(msm_camera_resources),
	.flash_data		= &msm_flash_vx6953,
	.strobe_flash_data	= NONE, //No strobe flash
	.csi_if			= 1
};
...
#define NFC_ON_DOWNLOAD		0
#define NFC_ON			1
#define NFC_OFF			2

static struct pn544_pm8058_gpio_cfg
{
	int gpio;
	struct pm8058_gpio cfg;
} pn544_pmic_gpio_cfgs[] = {
...
static int pn544_chip_power(unsigned long arg)
{
	if ((board_type >= OPAL_EVT3) || (board_type >= OPAL_3G_EVT3)) {
		switch (arg) {
		case NFC_ON_DOWNLOAD:
			pr_info("%s power on with firmware\n", __func__);
			gpio_set_value_cansleep(PM8058_GPIO_PM_TO_SYS(
				PM8058_NFC_GPIO4-1), 1);
			msleep(10);
			gpio_set_value_cansleep(PM8058_GPIO_PM_TO_SYS(
				PM8058_NFC_WAKEUP-1), 1);
			msleep(10);
			break;
		case NFC_ON:
			pr_info("%s power on\n", __func__);
			gpio_set_value_cansleep(PM8058_GPIO_PM_TO_SYS(
				PM8058_NFC_GPIO4-1), 0);
			gpio_set_value_cansleep(PM8058_GPIO_PM_TO_SYS(
				PM8058_NFC_WAKEUP-1), 1);
			msleep(10);
			break;
...
/*     The Nokia driver does not return a value which does not allow the
 *     driver to detect if enable succeeds or fails.  However, this is OK,
 *     because this testing should be done in the probe, if the probe
 *     succeeds, the enable must succeed.
 */
static void pn544_enable(int fw)
{
	if (fw)
		pn544_chip_power(NFC_ON_DOWNLOAD);
	else {
		pn544_chip_power(NFC_OFF);
		pn544_chip_power(NFC_ON);
	}
}
...
static int pn544_irq_get(void)
{
	if ((board_type >= OPAL_EVT3) || (board_type >= OPAL_3G_EVT3))
		return gpio_to_irq(PM8058_GPIO_PM_TO_SYS(
				PM8058_NFC_IRQOUT-1));
	else
		return -EPERM;
}

static int pn544_gpio_cfg(struct i2c_client *client)
{
	int rc = -1;
	int i;

	for (i = 0; i < ARRAY_SIZE(pn544_pmic_gpio_cfgs); ++i) {
		rc = pm8058_gpio_config(pn544_pmic_gpio_cfgs[i].gpio,
			&pn544_pmic_gpio_cfgs[i].cfg);
		if (rc < 0) {
	...
	rc = gpio_request(PM8058_GPIO_PM_TO_SYS(
			PM8058_NFC_IRQOUT-1), "nfc_int");
	if (rc < 0) {
		pr_err("NFC IRQ fail\n");
		return rc;
	}
	...
	rc = pn544_irq_get();
	if (rc < 0) {
		pr_err("NFC IRQ mapping fail\n");
		goto err_nfc_int;
	} else {
		client->irq = rc;
	}

	gpio_direction_input(PM8058_GPIO_PM_TO_SYS(
				PM8058_NFC_IRQOUT-1));

	rc = gpio_request(PM8058_GPIO_PM_TO_SYS(
				PM8058_NFC_WAKEUP-1), "nfc_ven");
	if (rc < 0) {
		pr_err("NFC reset fail\n");
		goto err_nfc_ven;
	}

	rc = gpio_request(PM8058_GPIO_PM_TO_SYS(
				PM8058_NFC_GPIO4-1), "nfc_firm");
	if (rc < 0) {
		pr_err("NFC firmware io config fail\n");
		goto err_nfc_firm;
	}
...
err_nfc_firm:
	gpio_free(PM8058_GPIO_PM_TO_SYS(PM8058_NFC_WAKEUP-1));
err_nfc_ven:
err_nfc_int:
	gpio_free(PM8058_GPIO_PM_TO_SYS(PM8058_NFC_IRQOUT-1));

	return rc;
}

static void pn544_gpio_free(void)
{
	if ((board_type >= OPAL_EVT3) || (board_type >= OPAL_3G_EVT3)) {
		gpio_free(PM8058_GPIO_PM_TO_SYS(PM8058_NFC_GPIO4-1));
		gpio_free(PM8058_GPIO_PM_TO_SYS(PM8058_NFC_WAKEUP-1));
		gpio_free(PM8058_GPIO_PM_TO_SYS(PM8058_NFC_IRQOUT-1));
	}
}

static int pn544_irq_trigger_type(void)
{
...
{
	/* Stuff test routine here. */
	return 0;
}

struct pn544_nfc_platform_data  pn544_gpio_ctrl = {
	.request_resources = pn544_gpio_cfg,
	.free_resources = pn544_gpio_free,
	.irq_trigger_type =  pn544_irq_trigger_type,
	.enable = pn544_enable,
	.disable = pn544_disable,
	.test = pn544_test,
};

static struct i2c_board_info pn544_i2c_info[] = {
	{
		I2C_BOARD_INFO("pn544", 0x28),
		.platform_data = &pn544_gpio_ctrl,
		.irq = PM8058_GPIO_IRQ(PM8058_IRQ_BASE, PM8058_NFC_IRQOUT-1),
	}
};
#endif
...
static int gsbi5_spi_gpios[] = {
	49, /* GSBI5_SPI_OUT */
	51, /* GSBI5_SPI_CS*/
	52, /* GSBI5_SPI_CLK*/
};

static int gsbi5_spi_gpio_config(void)
{
	return configure_gpiomux_gpios(1, gsbi5_spi_gpios, ARRAY_SIZE(gsbi5_spi_gpios));
}

static void gsbi5_spi_gpio_release(void)
{
	configure_gpiomux_gpios(0, gsbi5_spi_gpios, ARRAY_SIZE(gsbi5_spi_gpios));
}
...
static struct msm_spi_platform_data msm_gsbi5_qup_spi_pdata = {
	...
	.clk_name = "gsbi_qup_clk",
	.pclk_name = "gsbi_pclk",
	.gpio_config = gsbi5_spi_gpio_config,
	.gpio_release = gsbi5_spi_gpio_release,
};
...
static struct hsuart_platform_data ctp_uart_data = {
	...
	.uart_speed = CTP_UART_SPEED_FAST,
...
static struct msm_panel_common_pdata lcdc_common_panel_data = {
	...
	.pmic_backlight = lcdc_hitachi_pmic_bl,
};
...
{
	return MXT1386_TS_PEN_IRQ_GPIO_3G;
}
...
{
	return gpio_get_value(MXT1386_TS_PEN_IRQ_GPIO_3G);
}
...
#endif //CONFIG_TOUCHSCREEN_MXT1386_I2C
...
static struct bluetooth_power_state_platform_data bt_power_state_shortloin = {
	.dev_name = "bt_power",
	.bt_power = bt_power
};
...
static struct platform_device bluetooth_power_state_device = {
	...
	.id = 0,
	.dev = {
		.platform_data = &bt_power_state_shortloin
	}
};
...
static int cy8ctma395_tp_init(int on)
{
	int ret;
...
static struct platform_device *shortloin_devices_early[] __initdata = {
	&msm_device_smd,
...
static struct platform_device *shortloin_devices[] __initdata = {
#ifdef CONFIG_I2C_QUP
...
static struct wm8994_retune_mobile_cfg shortloin_retune_mobile_cfgs[] = {
	{
		.name = "AIF1 DAC EQ for Speaker",
		.rate = 48000,
		.regs = {
			0x48dd, 0x4b00, 0x0f01, 0x03fe, 0x03fc,
			0x1eb9, 0xf0ee, 0x040a, 0x03af, 0x1cbf,
			0xf1d9, 0x040b, 0x0751, 0x182d, 0xf349,
			0x040b, 0x0d03, 0x0381, 0x04e0, 0x4000,
			},
	}
};

static struct wm8958_mbc_cfg shortloin_mbc_cfgs[] = {
	{
		.name = "MBC Speaker",
		.combined_regs = {
					0x000f, 0xf64c, 0x0032, 0xf52d, 0x0065,
					0xac8c, 0x006b, 0xe087, 0x0072, 0x1483,
					0x0072, 0x1483, 0x004c, 0xc50a, 0x0019,
					0x8a13, 0x004f, 0xf3e1, 0x000c, 0xcccd,
					0x0000, 0x0800, 0x000a, 0x1247, 0x0032,
					0xf52d, 0x004c, 0x3ea8, 0x005f, 0x2995,
					0x0072, 0x1483, 0x0072, 0x1483, 0x0054,
					0x3d13, 0x0028, 0x7a27, 0x005d, 0x9923,
					0x000c, 0xcccd, 0x0000, 0x0800, 0x005a,
					0x7efa, 0x005a, 0x7efa, 0x008d, 0xce80,
					0x0087, 0x1833, 0x0080, 0x5d3d, 0x0008,
					0xe7a2,
				},
	}
};

static struct wm8994_drc_cfg  shortloin_drc_cfgs[] = {
	{
		.name = "Onboard Mic",
		.regs = {0x009a, 0x0840, 0x0001, 0x0000, 0x0000},
	}
};
...
static struct wm8994_pdata wm8958_pdata = {
	...
	.micdet_irq = SHORTLOIN_AUD_HEAD_MIC_DET_IRQ_GPIO,
	.micbias = { 0x0029, 0x002d, },
	.num_mbc_cfgs = 1,
	.mbc_cfgs = shortloin_mbc_cfgs,
	.num_retune_mobile_cfgs = 1,
	.retune_mobile_cfgs = shortloin_retune_mobile_cfgs,
	.num_drc_cfgs = 1,
	.drc_cfgs = shortloin_drc_cfgs,
};
...
static void __init register_i2c_devices(void)
{
	...
	if ((boardtype_is_3g() && board_type >= OPAL_3G_EVT3) ||
	(board_type >= OPAL_EVT3 && board_type < OPAL_DVT2 && !board_is_lite)) {
		i2c_register_board_info(MSM_GSBI9_QUP_I2C_BUS_ID,
					pn544_i2c_info,
...
{
#ifndef LCD_TYPE_IN_KERNEL_COMMAND_LINE
	if (board_type == OPAL_3G_PROTO)
	{
...
static void __init msm8x60_init_buses(void)
{
	...
	msm_device_uart_dm4.dev.platform_data = &msm_device_uart_dm4_pdata;
#endif
#ifdef CONFIG_MSM_BUS_SCALING
...
static void __init msm8x60_init_ebi2(void)
{
	...
	if (ebi2_cfg_ptr != 0) {
		...
		if (machine_is_msm8x60_surf() || machine_is_msm8x60_ffa() || machine_is_shortloin())
...
static uint32_t shortloin_tlmm_cfgs[] = {
...
static void __init msm8x60_init_tlmm(void)
{
	...
	for (n = 0; n < ARRAY_SIZE(shortloin_tlmm_cfgs); ++n)
		gpio_tlmm_config(shortloin_tlmm_cfgs[n], 0);
}
...
static int shortloin_wifi_power(int on)
{
	...
	int gpios[] = {
			SHORTLOIN_GPIO_WL_HOST_WAKE,
			SHORTLOIN_GPIO_HOST_WAKE_WL,
			SHORTLOIN_GPIO_WLAN_RST_N,
		};
	...
	if (on) {
		...
		gpio_direction_output(SHORTLOIN_GPIO_WLAN_RST_N, 0);
		mdelay(5);
		gpio_direction_output(SHORTLOIN_GPIO_WLAN_RST_N, 1);
	}
	else
	{
		//CHIP_PWD -> wait (Tc 5)
...
static void shortloin_probe_wifi(int id, struct mmc_host *mmc)
{
	printk("%s: id %d mmc %p\n", __PRETTY_FUNCTION__, id, mmc);
	wifi_mmc = mmc;
}

static void shortloin_remove_wifi(int id, struct mmc_host *mmc)
{
	printk("%s: id %d mmc %p\n", __PRETTY_FUNCTION__, id, mmc);
	wifi_mmc = NULL;
}
...
static struct mmc_platform_data msm8x60_sdc4_data = {
	...
	.translate_vdd  = msm_sdcc_setup_power,
	.mmc_bus_width  = MMC_CAP_4_BIT_DATA,
	.board_probe	= shortloin_probe_wifi,
	.board_remove	= shortloin_remove_wifi,
	.dummy52_required = 1,
	.msmsdcc_fmin	= 400000,
	.msmsdcc_fmid	= 24000000,
	.msmsdcc_fmax	= 48000000,
...
static int lcdc_common_panel_power(int on)
{
	...
	if (on) {
		...
		//we need to wait panel power to be stable before we send SPI command
		mdelay(13);
	} else {
		if (1 == isPowerOn) {
			mdelay(200);
		...
		/* We still need to disable backlight here till
		 * powerd turns off backlight before panel
		 */
...
static int lcdc_hitachi_pmic_bl(int bl_level)
{
	static int saved_bl_level = 1;

	if (!saved_bl_level && bl_level) {
		msleep(120);
		gpio_set_value_cansleep(GPIO_BACKLIGHT_EN, 1);
	} else if (saved_bl_level && !bl_level) {
		gpio_set_value_cansleep(GPIO_BACKLIGHT_EN, 0);
	}

	saved_bl_level = bl_level;
...
#define SHORTLOIN_CLOCK_FIXUP 1
#if SHORTLOIN_CLOCK_FIXUP
...
static struct msm_board_data shortloin_board_data __initdata = {
	.gpiomux_cfgs = shortloin_gpiomux_cfgs,
};

static int shortloin_pins_evt1 [] = {
	[SHORTLOIN_A6_0_TCK_PIN] = SHORTLOIN_A6_0_TCK_EVT1,
	[SHORTLOIN_A6_0_WAKEUP_PIN] = SHORTLOIN_A6_0_WAKEUP,
	[SHORTLOIN_A6_0_TDIO_PIN] = SHORTLOIN_A6_0_TDIO,
	[SHORTLOIN_A6_0_MSM_IRQ_PIN] = SHORTLOIN_A6_0_MSM_IRQ_EVT1,
	[SHORTLOIN_A6_1_TCK_PIN] = SHORTLOIN_A6_1_TCK,
	[SHORTLOIN_A6_1_WAKEUP_PIN] = SHORTLOIN_A6_1_WAKEUP,
	[SHORTLOIN_A6_1_TDIO_PIN] = SHORTLOIN_A6_1_TDIO,
	[SHORTLOIN_A6_1_MSM_IRQ_PIN] = SHORTLOIN_A6_1_MSM_IRQ_EVT1,
};

static int shortloin_pins [] = {
	[SHORTLOIN_A6_0_TCK_PIN] = SHORTLOIN_A6_0_TCK,
	[SHORTLOIN_A6_0_WAKEUP_PIN] = SHORTLOIN_A6_0_WAKEUP,
	[SHORTLOIN_A6_0_TDIO_PIN] = SHORTLOIN_A6_0_TDIO,
	[SHORTLOIN_A6_0_MSM_IRQ_PIN] = SHORTLOIN_A6_0_MSM_IRQ,
	[SHORTLOIN_A6_1_TCK_PIN] = SHORTLOIN_A6_1_TCK,
	[SHORTLOIN_A6_1_WAKEUP_PIN] = SHORTLOIN_A6_1_WAKEUP,
	[SHORTLOIN_A6_1_TDIO_PIN] = SHORTLOIN_A6_1_TDIO,
	[SHORTLOIN_A6_1_MSM_IRQ_PIN] = SHORTLOIN_A6_1_MSM_IRQ,
};
...
static void __init platform_fixup_pin_numbers(void)
{
	...
	// A6
	shortloin_a6_0_platform_data.pwr_gpio = pin_table[SHORTLOIN_A6_0_MSM_IRQ_PIN];
	shortloin_a6_0_platform_data.sbw_tck_gpio = pin_table[SHORTLOIN_A6_0_TCK_PIN];
	shortloin_a6_0_platform_data.sbw_tdio_gpio = pin_table[SHORTLOIN_A6_0_TDIO_PIN];
	shortloin_a6_0_platform_data.sbw_wkup_gpio = pin_table[SHORTLOIN_A6_0_WAKEUP_PIN];
	a6_0_config_data[0] = pin_table[SHORTLOIN_A6_0_MSM_IRQ_PIN];
	a6_0_sbw_gpio_config[0] = pin_table[SHORTLOIN_A6_0_TCK_PIN];
	a6_0_sbw_gpio_config[1] = pin_table[SHORTLOIN_A6_0_WAKEUP_PIN];
	a6_0_sbw_gpio_config[2] = pin_table[SHORTLOIN_A6_0_TDIO_PIN];

	shortloin_a6_1_platform_data.pwr_gpio = pin_table[SHORTLOIN_A6_1_MSM_IRQ_PIN];
	shortloin_a6_1_platform_data.sbw_tck_gpio = pin_table[SHORTLOIN_A6_1_TCK_PIN];
	shortloin_a6_1_platform_data.sbw_tdio_gpio = pin_table[SHORTLOIN_A6_1_TDIO_PIN];
	shortloin_a6_1_platform_data.sbw_wkup_gpio = pin_table[SHORTLOIN_A6_1_WAKEUP_PIN];
	a6_1_config_data[0] = pin_table[SHORTLOIN_A6_1_MSM_IRQ_PIN];
	a6_1_sbw_gpio_config[0] = pin_table[SHORTLOIN_A6_1_TCK_PIN];
	a6_1_sbw_gpio_config[1] = pin_table[SHORTLOIN_A6_1_WAKEUP_PIN];
	a6_1_sbw_gpio_config[2] = pin_table[SHORTLOIN_A6_1_TDIO_PIN];
...
static void __init power_up_gyroscope(void)
{
	...
	int need_l12 =	(boardtype_is_3g() && board_type >= OPAL_3G_EVT3) ||
			(!boardtype_is_3g() && board_type >= OPAL_EVT3);
...
static struct notifier_block shortloin_reboot_notifier = {
	.notifier_call = board_reboot_call,
};

static void board_register_reboot_notifier()
{
	//We don't need the notifiler after EVT3 because HW will change GPIO82 to GPIO106 from EVT3
	if((board_type >= OPAL_3G_EVT1)&&(board_type < OPAL_3G_EVT3))
	{
		register_reboot_notifier(&shortloin_reboot_notifier);
	}
}
...
static void __init msm8x60_init(struct msm_board_data *board_data)
{
	...
	pin_table = &shortloin_pins[0];
	if((boardtype_is_3g() && board_type < OPAL_3G_EVT2) ||
	   (!boardtype_is_3g() && board_type < OPAL_EVT2))
		pin_table = &shortloin_pins_evt1[0];
	...
	if(boardtype_is_3g()) {
		if(board_type == OPAL_3G_PROTO)
			msm8x60_init_gpiomux(shortloin_3g_gpiomux_cfgs_proto);
		else if(board_type == OPAL_3G_EVT1)
			msm8x60_init_gpiomux(shortloin_3g_gpiomux_cfgs_evt1);
		else
			msm8x60_init_gpiomux(shortloin_3g_gpiomux_cfgs);
	} else {
		msm8x60_init_gpiomux(shortloin_gpiomux_cfgs);
	}

	platform_add_devices(shortloin_devices_early,
					ARRAY_SIZE(shortloin_devices_early));

	msm8x60_init_mmc();

#if defined (CONFIG_TOUCHSCREEN_CY8CTMA395) \
	|| defined (CONFIG_TOUCHSCREEN_CY8CTMA395_MODULE)
	cy8ctma395_tp_init (true);
#endif
	...
	platform_add_devices(shortloin_devices,
					ARRAY_SIZE(shortloin_devices));
	register_lcdc_panel();

	// Make sure the reference counters for LCD power controllers is correct
	lcdc_common_panel_power(1);
...
#ifdef CONFIG_BRCM4751
	if (boardtype_is_3g() == false && !board_is_lite) {
		platform_device_register(&msm_device_uart_dm4);
		platform_device_register(&brcm4751_device);
	}
#endif
...
static void __init shortloin_init(void)
{
	msm8x60_init(&shortloin_board_data);
}

MACHINE_START(SHORTLOIN, "SHORTLOIN")
