...
static struct platform_device *snd_devices_shortloin[] __initdata = {
	&msm_ispkr_stereo_device,
	&msm_ispkr_mic_device,
	&msm_phone_rx_device,
	&msm_phone_tx_device,
...
static struct platform_device *snd_devices_crespo[] __initdata = {
	&msm_ispkr_stereo_device,
	&msm_ispkr_mic_device,
	&msm_phone_rx_device,
	&msm_phone_tx_device,
...
void __init msm_snddev_init(void)
{
	...
	} else if (machine_is_shortloin()) {
		for (i = 0; i < ARRAY_SIZE(snd_devices_shortloin); i++)
			snd_devices_shortloin[i]->id = dev_id++;

		platform_add_devices(snd_devices_shortloin,
		ARRAY_SIZE(snd_devices_shortloin));
	...
	}  else if (machine_is_crespo()) {
		for (i = 0; i < ARRAY_SIZE(snd_devices_crespo); i++)
			snd_devices_rump[i]->id = dev_id++;

		platform_add_devices(snd_devices_crespo,
		ARRAY_SIZE(snd_devices_crespo));
	}
