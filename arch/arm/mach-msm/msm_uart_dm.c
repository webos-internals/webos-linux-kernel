...
#include <asm/arch/msm_uart_dm.h>
...
static void 
msm_uartdm_reset(struct generic_uart_port *port);
...
msm_uartdm_set_rx_flow(struct generic_uart_port* i_p_port, uint32_t flow_ctl, uint32_t flow_state)
{
	...
	MSM_UARTDM_DEBUG("%s, port0x%x, flow %d\n",
			__FUNCTION__, (uint32_t)i_p_port, flow);
...
*
*/
static int __msm_uartdm_port_init(struct generic_uart_port* io_p_uart_port)
{
	int 		ret = 0;
	u32		rfr_level;
	unsigned int	data;
	struct msm_uart_port*	p_msm_port = GEN_UART_TO_MSM(io_p_uart_port);
	 ...
	 */
	clk_enable(io_p_uart_port->p_clk);

	/* 
	 * Turn off the UART interrupts
	 */
	msm_write(io_p_uart_port, 0, UART_DM_IMR);

	//Mux the relevant pins as functional
	if (io_p_uart_port->p_board_pin_mux_cb) {
		io_p_uart_port->p_board_pin_mux_cb(1);
	}

	/* 
	 * Set the UART speed 
	 */
	__msm_uartdm_set_baud_rate(io_p_uart_port, io_p_uart_port->baud_rate);

	/* Reset UART */
	/* TODO: amir - move to a function to do more generic configuraion */
	msm_write(io_p_uart_port, 
		  UART_DM_MR2_BITS_PER_CHAR_8 | UART_DM_MR2_STOP_BIT_LEN_ONE | io_p_uart_port->parity_data,
		  UART_DM_MR2);	/* 8N1 */

	/* Configure RFR for proper flow control */
	if (likely(io_p_uart_port->rx_fifo_size > 12)) {
		rfr_level = io_p_uart_port->rx_fifo_size - 12;
	}
	else {
		rfr_level = io_p_uart_port->rx_fifo_size;
	}

	/* set automatic RFR level */
	data = msm_read(io_p_uart_port, UART_DM_MR1);
	...
	msm_write(io_p_uart_port, data, UART_DM_MR1);

	/* Set flow control */
	msm_uartdm_set_rx_flow(io_p_uart_port, p_msm_port->rx_flow_ctl, p_msm_port->rx_flow_state);
	msm_uartdm_set_tx_flow(io_p_uart_port, p_msm_port->tx_flow_ctl);

	/* Configure stale settings */
	/* make sure that RXSTALE count is non-zero */
	data = msm_read(io_p_uart_port, UART_DM_IPR);
	if (unlikely(!data)) {
		data |= UART_DM_IPR_STALE_TIMEOUT_LSB_MSK;
		msm_write(io_p_uart_port, data, UART_DM_IPR);
	}

	if ( RX_MODE_DM(p_msm_port) ) {
		msm_write( io_p_uart_port, UART_DM_DMEN_RX_DM_EN, UART_DM_DMEN  );
	}
	/* Reset UART */
	msm_uartdm_reset(io_p_uart_port);
...
static int __msm_uartdm_port_init_imr(struct generic_uart_port* io_p_uart_port)
{
	int ret = 0;
	struct msm_uart_port*	p_msm_port = GEN_UART_TO_MSM(io_p_uart_port);
...
*
*/
int msm_uartdm_port_resume(struct generic_uart_port* io_p_uart_port)
{
	int ret = 0;

	MSM_UARTDM_ENTER();
	ret = __msm_uartdm_port_init(io_p_uart_port); 
	...
	ret = __msm_uartdm_port_init_imr(io_p_uart_port);
end:
	MSM_UARTDM_EXIT();
...
msm_uartdm_port_init(struct generic_uart_port*	io_p_uart_port)
{
	...
	/* Register for IRQ */
	ret = request_irq(io_p_uart_port->irq, 
			msm_uartdm_irq, 
			IRQF_TRIGGER_HIGH,
			p_msm_port->name, 
			io_p_uart_port);
...
msm_uartdm_reset(struct generic_uart_port* i_p_port)
{
	...
	msm_uartdm_set_rx_flow(i_p_port, p_msm_port->rx_flow_ctl, p_msm_port->rx_flow_state);

	msm_write(i_p_port, UART_DM_CR_CMD_RESET_TX, UART_DM_CR);
	/* Restore tx flow control */
	msm_uartdm_set_tx_flow(i_p_port, p_msm_port->tx_flow_ctl);
...
#if 0
static void msm_enable_clock(struct generic_uart_port *port)
{
	struct msm_uart_port* p_msm_uart_port = GEN_UART_TO_MSM(port);

	clk_enable(p_msm_uart_port->p_clk);
}
static void msm_shutdown(struct generic_uart_port *port)
{
	struct msm_uart_port *msm_uart_port = GEN_UART_TO_MSM(port);

	msm_uart_port->imr = 0;
	msm_write(port, 0, UART_DM_IMR); /* disable interrupts */

	free_irq(port->irq, port);
}
TODO: amir, add platform_device parameter to these functions 
static void msm_release_port(struct generic_uart_port *port)
{
...
static int msm_request_port(struct generic_uart_port *port)
{
	struct platform_device *pdev = to_platform_device(port->dev);
	struct resource *resource;
	resource_size_t size;
...
void 
msm_uartdm_send_dword(struct generic_uart_port* i_p_port, int data)
{
...
msm_uartdm_get_dword(struct generic_uart_port* i_p_port)
{
	...
	return (int)msm_read(i_p_port, UART_DM_RF);
}
...
//TODO: amir, add alternative function to get the UART control structure from the id 
	...
	p_msm_port->p_clk = clk_get(NULL, p_msm_port->p_clk_name);
	if (unlikely(NULL == p_msm_port->p_clk)) {
		ret =  -ENXIO;
		line_number = __LINE__;
		goto probe_err_invalid_param;
	}
	p_port->clk_rate = clk_get_rate(p_msm_port->p_clk);
...
{
/*
	struct msm_uart_port *msm_uart_port;

	if (unlikely(pdev->id < 0 || pdev->id >= UART_NR))
		return -ENXIO;

	msm_uart_port = GEN_UART_TO_MSM(get_port_from_line(pdev->id));

	clk_disable(msm_uart_port->clk);
	clk_put(msm_uart_port->clk);
*/
	return 0;
}
