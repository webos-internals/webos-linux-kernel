...
static int vreg_get_stats(char *buffer, const int bufmax)
{
	int n = 0;
    ...
    for(i=0; i < ARRAY_SIZE(vregs); i++)
    {
        ...
        if(msm_proc_comm(PCOM_PALM_DIAG_CMDS, &cmd_status, &id_status)) {
			n += scnprintf(buffer + n, bufmax - n, "  %s N/A\n", vregs[i].name);
        }
        else {
			bool enabled = !!id_status;

            if(enabled || vregs[i].refcnt) {
				n += snprintf(buffer + n, bufmax - n, "  %s [%s] apps_count=%d\n",
						vregs[i].name, 
...
void vreg_print_stats(void)
{
	int n;
	char buffer[4096];
	n = vreg_get_stats(buffer, ARRAY_SIZE(buffer));
	printk("%s", buffer);
}

static int vreg_stats_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	int n;
	char buffer[4096];
	n = vreg_get_stats(buffer, ARRAY_SIZE(buffer));
...
static const struct file_operations vreg_stats_fops= {
	.read = vreg_stats_read,
	.open = NULL,
};
...
static int __init vreg_debug_init(void)
{
	...
	debugfs_create_file("stats", 0644, dent, NULL, &vreg_stats_fops);
