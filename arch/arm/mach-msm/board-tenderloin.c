...
#ifndef CONFIG_FB_MSM_LCDC_LG_XGA_PANEL
...
#define MSM_FB_SIZE 0x8A5000
#else /* CONFIG_FB_MSM_HDMI_MSM_PANEL */
#define MSM_FB_SIZE 0x900000
#endif /* CONFIG_FB_MSM_HDMI_MSM_PANEL */
#endif /* CONFIG_FB_MSM_LCDC_LG_XGA_PANEL */
...
static struct msm_touchpad_platform_data msm_touchscreen_data = {
    .gpioirq     = MXT1386_TS_PEN_IRQ_GPIO,
};

static struct i2c_board_info xMT1386_board_info[] = {
    {
        I2C_BOARD_INFO("mXT1386", 0x4C),
...
#define UART_PMUX_CFG_ACTIVE(gpio) GPIO_CFG(gpio, 1, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_4MA)
#define UART_PMUX_CFG_SLEEP(gpio) GPIO_CFG(gpio, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA)

static int btuart_pin_mux(int on)
{
	if (on) {
		gpio_tlmm_config(UART_PMUX_CFG_ACTIVE(UART1DM_CTS_GPIO), GPIO_CFG_ENABLE);
		gpio_tlmm_config(UART_PMUX_CFG_ACTIVE(UART1DM_RX_GPIO), GPIO_CFG_ENABLE);
		gpio_tlmm_config(UART_PMUX_CFG_ACTIVE(UART1DM_TX_GPIO), GPIO_CFG_ENABLE);
	} else {
		gpio_tlmm_config(UART_PMUX_CFG_SLEEP(UART1DM_RTS_GPIO), GPIO_CFG_DISABLE);
		gpio_tlmm_config(UART_PMUX_CFG_SLEEP(UART1DM_CTS_GPIO), GPIO_CFG_DISABLE);
		gpio_tlmm_config(UART_PMUX_CFG_SLEEP(UART1DM_RX_GPIO), GPIO_CFG_DISABLE);
		gpio_tlmm_config(UART_PMUX_CFG_SLEEP(UART1DM_TX_GPIO), GPIO_CFG_DISABLE);
	}
...
static int btuart_deassert_rts(int deassert)
{
	if (deassert) {
		...
		gpio_tlmm_config(UART_PMUX_CFG_ACTIVE(UART1DM_RTS_GPIO), GPIO_CFG_ENABLE);
	}
...
#define BT_PMUX_CFG_OUT(gpio) GPIO_CFG(gpio, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA)
#define BT_PMUX_CFG_IN(gpio) GPIO_CFG(gpio, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA)
...
static int bt_power(unsigned int on)
{
	if (on) {
		...
		gpio_tlmm_config(BT_PMUX_CFG_OUT(BT_RST_N), GPIO_CFG_ENABLE);
		gpio_tlmm_config(BT_PMUX_CFG_OUT(BT_POWER), GPIO_CFG_ENABLE);
		gpio_tlmm_config(BT_PMUX_CFG_OUT(BT_WAKE), GPIO_CFG_ENABLE);
		gpio_tlmm_config(BT_PMUX_CFG_IN(BT_HOST_WAKE), GPIO_CFG_ENABLE);
		...
		gpio_tlmm_config(BT_PMUX_CFG_OUT(BT_RST_N), GPIO_CFG_DISABLE);
		gpio_tlmm_config(BT_PMUX_CFG_IN(BT_HOST_WAKE), GPIO_CFG_DISABLE);
	}
...
static struct regulator_init_data pm8058_vreg_init[PM8058_VREG_MAX] = {
	...
	PM8058_VREG_INIT_LDO(PM8058_VREG_ID_L6,  3300000, 3300000),
	...
	PM8058_VREG_INIT_LDO(PM8058_VREG_ID_L10, 3050000, 3050000),
	...
	PM8058_VREG_INIT_SMPS(PM8058_VREG_ID_S2, 1200000, 1200000),
...
static struct regulator_init_data pm8901_vreg_init[PM8901_VREG_MAX] = {
	...
	PM8901_VREG_INIT_SMPS(PM8901_VREG_ID_S0, 1200000, 1200000),
    PM8901_VREG_INIT_SMPS(PM8901_VREG_ID_S1, 1200000, 1200000),
    PM8901_VREG_INIT_SMPS(PM8901_VREG_ID_S2, 1100000, 1100000),
...
#endif
	/* TENDERLON_GPIO_HOST_WAKE_WL */
	GPIO_CFG(137, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_8MA),
        /* TENDERLON_GPIO_WL_IRQ */
	GPIO_CFG(94,  0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_8MA),
	/* TENDERLON_GPIO_WL_HOST_WAKE */
	GPIO_CFG(93,  0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_8MA),
	/* TENDERLON_GPIO_WLAN_RST_N */
	GPIO_CFG(135, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA),
...
/*WLAN*/
#define TENDERLON_GPIO_WL_HOST_WAKE		93		// input
#define	TENDERLON_GPIO_WL_IRQ			94		// input
#define TENDERLON_GPIO_HOST_WAKE_WL		137		// output
#define TENDERLON_GPIO_WLAN_RST_N		135		// output
...
{	
	...
	if (on) 
	{
		...
		rc = gpio_request(TENDERLON_GPIO_WLAN_RST_N, "WLAN_RST_N");
		if (rc) 
		{
		    pr_err("%s:  gpio %d request failed\n",	__func__, TENDERLON_GPIO_WLAN_RST_N);
		    gpio_free(TENDERLON_GPIO_WLAN_RST_N);
		    return -EINVAL;
		}

		gpio_direction_output(TENDERLON_GPIO_WLAN_RST_N, 0);

		mdelay(5);

		gpio_direction_output(TENDERLON_GPIO_WLAN_RST_N, 1);
		//gpio_set_value(TENDERLON_GPIO_WLAN_RST_N, 1);
...
static uint32_t lcd_panel_gpios[] = {
	...
	GPIO_CFG(70, 0, GPIO_CFG_OUTPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_2MA),  /* TOUCH reset */
};
...
static void lcdc_lg_panel_power(int on)
{
	int n;
    ...
    {
        pr_err("%s: unable to get vot_vdd5v\n", __func__);
        return;
    }
    ...
    {   
       ...
       /* VDD_BACKLIGHT_5.0V ENABLE*/
        ...
        if (rc) 
        {
            printk(KERN_ERR "%s: LCD Power gpio %d request"
                        "failed\n", __func__,
                         GPIO_LCD_PWR_EN);
        ...
        gpio_direction_output(GPIO_LVDS_SHDN_N, 0);
	...
	for (n = 0; n < ARRAY_SIZE(lcd_panel_gpios); ++n)
		gpio_tlmm_config(lcd_panel_gpios[n], on);
}
#endif
