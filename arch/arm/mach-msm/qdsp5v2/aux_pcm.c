...
int aux_pcm_gpios_request(void)
{
	...
	configure_aux_pcm_pins(GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL);

	gpio_set_value(the_aux_pcm_state.dout, 1);
	gpio_set_value(the_aux_pcm_state.din, 1);
	gpio_set_value(the_aux_pcm_state.syncout, 1);
	gpio_set_value(the_aux_pcm_state.clkin_a, 1);
...
void aux_pcm_gpios_free(void)
{
	...
	gpio_set_value(the_aux_pcm_state.din, 0);
	gpio_set_value(the_aux_pcm_state.syncout, 0);
	gpio_set_value(the_aux_pcm_state.clkin_a, 0);

	configure_aux_pcm_pins(GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN);
...
void configure_aux_pcm_pins(int dir, int pull)
{
	int func = 0;

	if (dir == GPIO_CFG_OUTPUT)
		func = 1;

	gpio_tlmm_config(GPIO_CFG(the_aux_pcm_state.dout, func, dir,
			pull, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	gpio_tlmm_config(GPIO_CFG(the_aux_pcm_state.din, func, dir,
			pull, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	gpio_tlmm_config(GPIO_CFG(the_aux_pcm_state.syncout, func, dir,
			pull, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	gpio_tlmm_config(GPIO_CFG(the_aux_pcm_state.clkin_a, func, dir,
			pull, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
}
