...
static int proc_comm_wait_for(unsigned addr, unsigned value)
{
	...
	while (1) {
		...
		/* watch how long we wait and issue a warning if we seem to be stuck */
		dot_count++;
		if (dot_count >= 1000000) {
			dot_count = 0;
			msg_count++;
			printk(KERN_ERR ".");
		}
		if (msg_count == 30) {
			msg_count = 0;
			printk(KERN_ERR "%s: potentially stuck waiting for proc comm at addr=%08x; want %u, got %u\n", __FUNCTION__, addr, value, readl(addr));
		}
