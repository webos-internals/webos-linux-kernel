...
extern void smem_log_print(void);

#define SMEM_OUT_OF_SYNC_FATAL 0
#define SMEM_OUT_OF_SYNC_FATAL_DEBUG 1
...
{
	uint32_t gpio = msm_pm_smem_data->reserved2;
...
void log_wakesource(uint32_t wakereason)
{
	...
	if (wakereason & DEM_WAKEUP_REASON_GPIO) {
		if (GPIO_POWER == gpio)
			wakeup_event(wakeup_source_button, "POWER, GPIO=%d", gpio);
		else
			wakeup_event(wakeup_source_gpio, "GPIO=%d", gpio);
	}
	if (wakereason & DEM_WAKEUP_REASON_TIMER)
	...
	(bool from_idle, uint32_t sleep_delay, uint32_t sleep_limit)
{
	...
	if (MSM_PM_DEBUG_POWER_COLLAPSE & msm_pm_debug_mask) {
		...
		printk("===========================\n");
		printk("Clock state before sleep.\n");
		clock_print_stats(); 
		printk("\n");
		printk("Vreg state before sleep.\n");
		vreg_print_stats();
		printk("===========================\n");
	}
	 ...
	 * modem could raise early exit that could be ignored by APPS. 
	 *
	 * The following checks expose such problems,
	 * but the conditions seem to be harmless. As unhandled early exit conditions
	 * from the modem would cause modem to wake APPS up even if
	 * APPS goes into complete power collapse. Respective states for
	 * modem and APPS would be synchronized properly after this function
	 */

	if (collapsed) {

#if SMEM_OUT_OF_SYNC_FATAL
		BUG_ON(!(state_grps[0].value_read & DEM_MASTER_SMSM_RSA));
#else
		if(!(state_grps[0].value_read & DEM_MASTER_SMSM_RSA)) {
			printk(KERN_ERR "SMEM state out-of-sync after power collpase: "
				"!( %08x & DEM_MASTER_SMSM_RSA )\n", state_grps[0].value_read);

			smem_log_print();

#if SMEM_OUT_OF_SYNC_FATAL_DEBUG
			msm_proc_comm_reset_modem_now();
			BUG();
#endif
		}
		...
		if(!(state_grps[0].value_read & DEM_MASTER_SMSM_PWRC_EARLY_EXIT)) {
			printk(KERN_INFO "SMEM state out-of-sync after power collpase failed: "
				"!( %08x & DEM_MASTER_SMSM_PWRC_EARLY_EXIT )\n", state_grps[0].value_read);
...
#else
	/* Graceful reset chip is broken, fallback to chip reset immediate */
	...
	(struct notifier_block *this, unsigned long code, void *_cmd)
{
	if ((code == SYS_RESTART) && _cmd) {
		...
		// Palm specific
			...
			restart_reason = RESTART_REASON_UPDATE;
		} else {
