...
struct clk *clk_get(struct device *dev, const char *id)
{
	...
	// PALM: Removed check for dev match
...
static void clock_debug_rate_set(void *data, u64 val)
{
...
static u64 clock_debug_rate_get(void *data)
{
	struct clk *clock = data;
	return clk_get_rate(clock);
}

static void clock_debug_enable_set(void *data, u64 val)
{
	struct clk *clock = data;
	int rc = 0;
...
static u64 clock_debug_enable_get(void *data)
{
	struct clk *clock = data;
	return clock->ops->is_enabled(clock->id);
}
