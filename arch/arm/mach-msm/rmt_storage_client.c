...
static int rmt_storage_client_suspend(struct platform_device *pdev, pm_message_t state)
{
	//Ensure all pending replies go back to the modem before suspend
	if (_rmc && atomic_read(&_rmc->wcount) > 0) {
		printk(KERN_ERR"%s: Pending replies: %d\n", 
			__func__,atomic_read(&_rmc->wcount));
		return -EBUSY;
	}
...
static int rmt_storage_client_resume(struct platform_device *pdev)
{
	return 0;
}
...
static void rmt_storage_client_suspend NULL
static void rmt_storage_client_resume NULL
...
static struct platform_driver rmt_storage_driver = {
	...
	.suspend = rmt_storage_client_suspend,
	.resume = rmt_storage_client_resume,
	.driver	= {
		.name 	= "rs00000000",
		.owner	= THIS_MODULE,
	},
