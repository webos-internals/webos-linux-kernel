...
static uint8_t smd_package_version = SMD_PKG_4;
static uint8_t smd_num_hosts       = SMSM_NUM_HOSTS;


/* New modem baseline versions use the new NUM_HOSTS interface */
static uint8_t smsm_get_num_hosts()
{
	enum socinfo_interface socinfo_if = socinfo_get_interface();

	if ( socinfo_if == SOCINFO_INTERFACE_V2) {
		return SMSM_NUM_HOSTS_NEW;
	} else if (socinfo_if == SOCINFO_INTERFACE_V1) {
		return SMSM_NUM_HOSTS;
	} else {
		return SMSM_NUM_HOSTS;
	}
}

/* New modem baseline versions use SMD package 3,
 * while older ones use SMD package version4.
 */
static uint8_t smd_detect_package_version()
{
	enum socinfo_interface socinfo_if = socinfo_get_interface();
			      ...
			      uint32_t old_val, uint32_t new_val)
{
	...
	smsm_intr_mask = smem_alloc(SMEM_SMSM_CPU_INTR_MASK,
				    SMSM_NUM_ENTRIES * smd_num_hosts *
				    sizeof(uint32_t));

	/* older protocol don't use smsm_intr_mask,
	   but still communicates with modem */
	if (!smsm_intr_mask ||
	    (smsm_intr_mask[smsm_entry * smd_num_hosts + SMSM_MODEM] &
	    (old_val ^ new_val)))
		writel(1, MSM_A2M_INT(5));

	if (smsm_intr_mask &&
	    (smsm_intr_mask[smsm_entry * smd_num_hosts + SMSM_Q6_I] &
	    (old_val ^ new_val))) {
...
#define SMD_PKG3_CHANNEL_TYPE(x) ((x) & 0x000000FF)
#define SMD_PKG3_XFER_TYPE(x)    (((x) & 0x00000F00) >> 8)

struct smd_alloc_elm_smd_pkg4 {
	char name[20];
	uint32_t cid;
	uint32_t ctype;
	uint32_t cprotocol;
...
struct smd_channel
{
	...
	wait_queue_head_t serial_state_wait;
	unsigned char cnt_dcd, cnt_dsr;
	unsigned char st_dcd, st_dsr;
};
...
static void smd_alloc_channel(const char *name, uint32_t cid, uint32_t type, uint32_t xfer_type);
static void *_smem_find(unsigned id, unsigned *size);

static void smd_channel_probe_worker(struct work_struct *work)
{
	struct smd_alloc_elm_smd_pkg3 *shared_smd_pkg3;
	struct smd_alloc_elm_smd_pkg4 *shared_smd_pkg4;
	unsigned n;

	if (SMD_PKG_3 == smd_package_version) {
		shared_smd_pkg3 = smem_find(ID_CH_ALLOC_TBL, sizeof(*shared_smd_pkg3) * 64);
		...
		for (n = 0; n < 64; n++) {
			...
			if (SMD_PKG3_CHANNEL_TYPE (shared_smd_pkg3[n].ctype) == SMD_MODEM_QDSP_I)
				continue;
			if (!shared_smd_pkg3[n].ref_count)
				continue;
			if (!shared_smd_pkg3[n].name[0])
				continue;

			smd_alloc_channel(shared_smd_pkg3[n].name,
					  shared_smd_pkg3[n].cid,
					  SMD_PKG3_CHANNEL_TYPE (shared_smd_pkg3[n].ctype),
					  SMD_PKG3_XFER_TYPE (shared_smd_pkg3[n].ctype) );
			smd_ch_allocated[n] = 1;
		}
	} else if (SMD_PKG_4 == smd_package_version) {
		shared_smd_pkg4 = smem_find(ID_CH_ALLOC_TBL, sizeof(*shared_smd_pkg4) * 64);
		...
		for (n = 0; n < 64; n++) {
			...
			/* channel should be allocated only if APPS
			   processor is involved */
			if (shared_smd_pkg4[n].ctype == SMD_MODEM_QDSP_I)
				continue;
			if (!shared_smd_pkg4[n].ref_count)
				continue;
			if (!shared_smd_pkg4[n].name[0])
				continue;

			smd_alloc_channel(shared_smd_pkg4[n].name,
					  shared_smd_pkg4[n].cid,
					  shared_smd_pkg4[n].ctype,
					  shared_smd_pkg4[n].ctype);
			smd_ch_allocated[n] = 1;
		}
	}
...
static irqreturn_t smd_irq_handler(int irq, void *data)
{
	...
	list_for_each_entry(ch, &smd_ch_list, ch_list) {
		...
		if (ch_is_open(ch)) {
			...
			do_wakeup_icount = 0;
			if (ch->recv->fCD != ch->st_dcd) {
				ch->st_dcd = ch->recv->fCD;
				ch->cnt_dcd++;
				do_wakeup_icount |= 1;
			}
			if (ch->recv->fDSR != ch->st_dsr) {
				ch->st_dsr = ch->recv->fDSR;
				ch->cnt_dsr++;
				do_wakeup_icount |= 1;
			}
			if (do_wakeup_icount)
				wake_up_interruptible(&ch->serial_state_wait);
		}
		tmp = ch->recv->state;
...
static int smd_is_packet(struct smd_channel *ch, uint32_t xfer_type)
{
	if (SMD_PKG_3 == smd_package_version) {
		if (xfer_type == 1) {
			return 0;
...
static void smd_alloc_channel(const char *name, uint32_t cid, uint32_t type, uint32_t xfer_type)
{
	...
	init_waitqueue_head(&ch->serial_state_wait);
...
int smd_get_serial_counters(smd_channel_t *ch,
			    unsigned int *cnt_dcd, unsigned int *cnt_dsr)
{
	unsigned long flags;
...
wait_queue_head_t *smd_get_serial_state_wait(smd_channel_t *ch)
{
	return &ch->serial_state_wait;
}
...
static int smem_init(void)
{
	...
	smsm = smem_alloc(SMEM_SMSM_CPU_INTR_MASK,
			  SMSM_NUM_ENTRIES * smd_num_hosts * sizeof(uint32_t));

	if (smsm) {
		smsm[SMSM_APPS_STATE * smd_num_hosts + SMSM_MODEM] =
								0xffffffff;
		smsm[SMSM_APPS_DEM_I * smd_num_hosts + SMSM_MODEM] =
								0xffffffff;
		smsm[SMSM_APPS_STATE * smd_num_hosts + SMSM_Q6_I] = 0xffffffff;
		smsm[SMSM_APPS_DEM_I * smd_num_hosts + SMSM_Q6_I] = 0xffffffff;
	}
...
static int debug_read_alloc_tbl(char *buf, int max)
{
	...
	if (smd_package_version == SMD_PKG_3) {
		shared_smd_pkg3 = smem_find(ID_CH_ALLOC_TBL, sizeof(struct smd_alloc_elm_smd_pkg3[64]));

		for (n = 0; n < 64; n++) {
			i += scnprintf(buf + i, max - i,
					   "name=%s cid=%d ctype=%d ref_count=%d\n",
					   shared_smd_pkg3[n].name,
					   shared_smd_pkg3[n].cid,
					   SMD_PKG3_CHANNEL_TYPE (shared_smd_pkg3[n].ctype),
					   shared_smd_pkg3[n].ref_count);
		}
	} else if (smd_package_version == SMD_PKG_4) {
		shared_smd_pkg4 = smem_find(ID_CH_ALLOC_TBL, sizeof(struct smd_alloc_elm_smd_pkg4[64]));

		for (n = 0; n < 64; n++) {
			i += scnprintf(buf + i, max - i,
					"name=%s cid=%d ctype=%d "
					"cprotocol=%d ref_count=%d\n",
					shared_smd_pkg4[n].name,
					shared_smd_pkg4[n].cid,
					shared_smd_pkg4[n].ctype,
					shared_smd_pkg4[n].cprotocol,
					shared_smd_pkg4[n].ref_count);
		}
	}
...
static int debug_read_intr_mask(char *buf, int max)
{
	...
	if (smsm)
		for (m = 0; m < SMSM_NUM_ENTRIES; m++) {
			...
			for (n = 0; n < smd_num_hosts; n++)
				i += scnprintf(buf + i, max - i,
					       "   host %d: 0x%08x",
					       n, smsm[m * smd_num_hosts + n]);
			i += scnprintf(buf + i, max - i, "\n");
		}
...
static int __init msm_smd_probe(struct platform_device *pdev)
{
	...
	// This is the only place, it is modified. If done from more places,
	// this needs to be synchronized.
	smd_package_version = smd_detect_package_version();
	smd_num_hosts       = smsm_get_num_hosts();
