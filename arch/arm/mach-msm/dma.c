...
{
	unsigned chan_id = id;	
	unsigned crci_mask = ( 1 << crci );	

	// This command currently works only for hsuart 1 and 
    // hsuart 2 rx related channels/crci
	if ( ( chan_id == DMOV_HSUART1_RX_CHAN && crci == DMOV_HSUART1_RX_CRCI) ||
             ( chan_id == DMOV_HSUART2_RX_CHAN && crci == DMOV_HSUART2_RX_CRCI)) {
	
		msm_proc_comm(PROC_COMM_RESET_CRCI_DBG, &chan_id, &crci_mask);
	} else {
