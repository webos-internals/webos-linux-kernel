...
static uint32_t msm_pm_sleep_limit;
...
void arch_idle(void)
{
	...
	int exit_stat;
#endif
	//int latency_qos = pm_qos_requirement(PM_QOS_INTERRUPT_LATENCY);
...
#ifdef CONFIG_MSM_IDLE_STATS
		ret = msm_clock_require_tcxo(msm_pm_clocks_no_tcxo_shutdown,
			NR_CLKS);
#else
...
static int msm_pm_enter(suspend_state_t state)
{
	...
	ret = msm_sleep(msm_pm_sleep_mode,
		msm_pm_max_sleep_time, (uint32_t)ret, 0);
	...
	if ((msm_pm_sleep_mode == MSM_PM_SLEEP_MODE_POWER_COLLAPSE_SUSPEND ||
		...
		id = ret ? MSM_PM_STAT_FAILED_SUSPEND : MSM_PM_STAT_SUSPEND;
...
static int msm_reboot_call(struct notifier_block *this, unsigned long code, void *_cmd)
{
	if((code == SYS_RESTART) && _cmd) {
		...
		// Palm specific
			...
			restart_reason = 0x6f656d11;
		} else if (!strcmp(cmd, "panic")) {
			restart_reason = 0x6f656d22;
		} else {
...
static int __init msm_pm_init(void)
{
	...
	if (msm_pm_sma.sleep_delay == NULL) {
		printk(KERN_ERR "msm_pm_init: failed get SLEEP_DELAY (smd id=%d)\n", 
