...
struct smd_ctl_dev {
	struct cdev cdev;
	char name[9];
	struct class_device *class_devicep;
...
static int __init smd_ctl_init(void)
{
	...
	for (i = 0; i < NUM_SMD_CTL_PORTS; ++i) {
		...
		smd_ctl_devp[i]->class_devicep =
		  class_device_create(smd_ctl_classp,
				      NULL,
				      (smd_ctl_number + i),
				      NULL,
				      DEVICE_NAME "%d",
		...
		if (IS_ERR(smd_ctl_devp[i]->class_devicep)) {
			printk(KERN_ERR "%s:%i:%s: "
			       "class_device_create() ENOMEM\n",
			       __FILE__,
			       __LINE__,
			       __func__);
			r = -ENOMEM;
 ...
 error2:
	if (i > 0) {
		while (--i >= 0) {
			...
			destroy_workqueue(smd_ctl_devp[i]->ctl_wq);
			kfree(smd_ctl_devp[i]);
			class_device_destroy(smd_ctl_classp,
