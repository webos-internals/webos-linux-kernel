...
static int wm8994_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (wm8994 && !wm8994->pdata->force_route) {
		...
		* when clocking is enabled */
		...
		dev_dbg(codec->dev, "AIF1_CLOCKING_1 value 0x%x", val);
		...
		dev_dbg(codec->dev, "AIF2_CLOCKING_1 value 0x%x", val);
...
static int wm8994_codec_probe(struct platform_device *pdev)
{
	...
	case WM8958:
 		...
 		if (wm8994->micdet_irq) {
			// BODGE - NEED TO FIX PLATFORM DATA
