...
static struct msm_audio* alloc_msm_audio(void)
{
	struct msm_audio *_this;
	_this = kzalloc(sizeof(struct msm_audio), GFP_KERNEL);
	if (_this) {
		atomic_set(&_this->_ref, 1);
	}
	return _this;
}

static void get_msm_audio(struct msm_audio *prtd)
{
	atomic_inc(&prtd->_ref);
}

static void __put_msm_audio(struct msm_audio *prtd)
{
	kfree(prtd);
}

static void put_msm_audio(struct msm_audio *prtd)
{
	if (atomic_dec_and_test(&prtd->_ref))
	{
		__put_msm_audio(prtd);
	}
}
...
static int msm_pcm_playback_close(struct snd_pcm_substream *substream)
{
	...
	/* QC ALSA driver will hang on snd_pcm_close calls from pulseaudio while waiting for eos_ack to be true.
