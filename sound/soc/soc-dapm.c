...
struct snd_soc_dapm_widget *dapm_find_widget(struct snd_soc_codec *codec, const char *pin, int ignored)
{
	struct snd_soc_dapm_widget *w;
...
static int snd_soc_dapm_weak_route(struct snd_soc_codec *codec,
				   const struct snd_soc_dapm_route *route)
{
	struct snd_soc_dapm_widget *source = dapm_find_widget(codec,
							      route->source,
							      true);
	struct snd_soc_dapm_widget *sink = dapm_find_widget(codec,
							    route->sink,
							    true);
	struct snd_soc_dapm_path *path;
	int count = 0;
	...
	if (!source) {
		dev_err(codec->dev, "Unable to find source %s for weak route\n",
			route->source);
		return -ENODEV;
	}

	if (!sink) {
		dev_err(codec->dev, "Unable to find sink %s for weak route\n",
			route->sink);
		return -ENODEV;
	}

	if (route->control || route->connected)
		dev_warn(codec->dev, "Ignoring control for weak route %s->%s\n",
	...
	if (count == 0)
		dev_err(codec->dev, "No path found for weak route %s->%s\n",
			route->source, route->sink);
	if (count > 1)
		dev_warn(codec->dev, "%d paths found for weak route %s->%s\n",
 ...
 */
int snd_soc_dapm_weak_routes(struct snd_soc_codec *codec,
			     const struct snd_soc_dapm_route *route, int num)
{
	int i, err;
	int ret = 0;
	...
	for (i = 0; i < num; i++) {
		err = snd_soc_dapm_weak_route(codec, route);
		if (err)
			ret = err;
		route++;
	}
